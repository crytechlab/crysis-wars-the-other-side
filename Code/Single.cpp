...
-------------------------------------------------------------------------
History:
- 11:9:2005   15:00 : Created by Mбrcio Martins
...
void CSingle::Update(float frameTime, uint frameId)
{
	...
	if (m_zoomtimeout > 0.0f && m_fireparams.autozoom)
	{
		...
		{
			m_zoomtimeout = 0.0f;
			if (pActor && pActor->IsClient() && pActor->GetScreenEffects() != nullptr)
				// Only start a zoom out if we're zooming in and not already zooming out
				if (pActor->GetScreenEffects()->HasJobs(pActor->m_autoZoomInID) && !pActor->GetScreenEffects()->HasJobs(pActor->m_autoZoomOutID))
				{
...
bool CSingle::IsValidAutoAimTarget(IEntity* pEntity, int partId /*= 0*/)
{
	...
	if (pActor && pActor->GetHealth() > 0.f && pActor->GetEntity()->GetAI() && pActor->GetEntity()->GetAI()->IsHostile(pPlayer->GetEntity()->GetAI(), false))
		return true;
	...
	if (pVehicle && pVehicle->GetStatus().health > 0.f && pVehicle->GetEntity()->GetAI() && pVehicle->GetEntity()->GetAI()->IsHostile(pPlayer->GetEntity()->GetAI(), false))
		return true;
...
void CSingle::UpdateAutoAim(float frameTime)
{
	...
	{
		pEntity = static_cast<IEntity*>(ray.pCollider->GetForeignData(PHYS_FOREIGN_ID_ENTITY));
...
{
	const IItemParamsNode* fire = params ? params->GetChild("fire") : nullptr;
	const IItemParamsNode* tracer = params ? params->GetChild("tracer") : nullptr;
	const IItemParamsNode* ooatracer = params ? params->GetChild("outofammotracer") : nullptr;
	const IItemParamsNode* recoil = params ? params->GetChild("recoil") : nullptr;
	const IItemParamsNode* spread = params ? params->GetChild("spread") : nullptr;
	const IItemParamsNode* actions = params ? params->GetChild("actions") : nullptr;
	const IItemParamsNode* muzzleflash = params ? params->GetChild("muzzleflash") : nullptr;
	const IItemParamsNode* muzzlesmoke = params ? params->GetChild("muzzlesmoke") : nullptr;
	const IItemParamsNode* muzzlesmokeice = params ? params->GetChild("muzzlesmoke_ice") : nullptr;
	const IItemParamsNode* reject = params ? params->GetChild("reject") : nullptr;
	const IItemParamsNode* spinup = params ? params->GetChild("spinup") : nullptr;
	const IItemParamsNode* heating = params ? params->GetChild("heating") : nullptr;
	const IItemParamsNode* dust = params ? params->GetChild("dust") : nullptr;
...
void CSingle::Activate(bool activate)
{
	...
	if (owner)
		if (!activate && owner->GetScreenEffects() != nullptr)
		{
...
{
	return !m_reloading && !m_reloadPending && (m_next_shot <= 0.0f) && (m_spinUpTime <= 0.0f) && (m_overheat <= 0.0f) && !m_pWeapon->IsBusy() && (!considerAmmo || !OutOfAmmo() || !m_fireparams.ammo_type_class || m_fireparams.clip_size == -1);
}
...
struct CSingle::EndReloadAction
{
	EndReloadAction(CSingle* _single, int zoomed, int reloadStartFrame)
		: single(_single),
		_zoomed(zoomed),
		_reloadStartFrame(reloadStartFrame) {};
...
{
	StartReload_SliderBack(CSingle* _single)
		: single(_single) {};
	CSingle* single;
...
void CSingle::StartReload(int zoomed)
{
	...
	if (m_pWeapon->IsOwnerFP())
	{
		...
		if (m_pWeapon->GetHostId() && animTime == 0)
			animTime = static_cast<uint>(m_fireparams.reload_time * 1000.0f);
		...
		time = static_cast<int>((MAX(0, ((m_pWeapon->GetCurrentAnimationTime(CItem::eIGS_FirstPerson))-m_fireparams.slider_layer_time))));
	}
	else
	{
...
{
	Shoot_SliderBack(CSingle* _single)
		: pSingle(_single) {};
	CSingle* pSingle;
...
struct CSingle::ZoomedCockAction
{
	...
	ZoomedCockAction(CSingle* _single)
		: pSingle(_single) {};
...
namespace
{
	...
	{
		CompareEntityDist(const Vec3& to)
			: m_to(to) {}
...
bool CSingle::CrosshairAssistAiming(const Vec3& firingPos, Vec3& firingDir, ray_hit* pRayhit)
{
	...
	IEntity* pEntity = pRayhit->pCollider ? gEnv->pEntitySystem->GetEntityFromPhysics(pRayhit->pCollider) : nullptr;
	...
	for (int i = 0, n = ents.size(); i < n; ++i)
	{
		...
		if (overlap && !pTarget)
		{
			...
			{
				IEntity* pFound = chkhit.pCollider ? gEnv->pEntitySystem->GetEntityFromPhysics(chkhit.pCollider) : nullptr;
				if (pFound != pEntity)
					continue;
			}
...
{
public:
	EndCockingAction(CSingle* _single)
		: pSingle(_single) {};
	CSingle* pSingle;
...
bool CSingle::InternalShoot(IEntityClass* spawn_ammo, bool resetAnimation, bool autoreload/* =true */, bool noSound /* =false */)
{
	...
	bool        zoomedCock = m_fireparams.unzoomed_cock && (m_pWeapon->IsZoomed() || m_pWeapon->IsZooming()) && (ammoCount != 1);
	...
	if (pAmmo)
	{
		...
		pAmmo->SetParams(m_pWeapon->GetOwnerId(), m_pWeapon->GetHostId(), m_pWeapon->GetEntityId(), m_pWeapon->GetFireModeIdx(GetName()), static_cast<int>(damage), pGameRules->GetHitTypeId(m_fireparams.hit_type.c_str()));
	...
	if (playerIsShooter && pActor->IsClient())
	{
		...
		if (m_fireparams.autozoom && pActor->GetScreenEffects() != nullptr && !pActor->GetScreenEffects()->HasJobs(pActor->m_autoZoomInID) && !pActor->GetScreenEffects()->HasJobs(pActor->m_autoZoomOutID))
		{
			CLinearBlend* blend = new CLinearBlend(1);
...
Vec3 CSingle::GetProbableHit(float range, bool* pbHit, ray_hit* pHit) const
{
	...
	IMovementController* pMC = pActor ? pActor->GetMovementController() : nullptr;
	if (pMC)
	{
		SMovementState info;
		...
		if (pbHit && *pbHit && pActor && pActor->GetLinkedVehicle())
		{
...

			{
				...
				if (dist < 0.0f)
				{
					...
					if (gEnv->pPhysicalWorld->RayWorldIntersection(newPos, dir, ent_all, rwi_stop_at_pierceable | rwi_ignore_back_faces, &hit, 1, pSkipEntities, nSkip))
					{
						if (pbHit)
							*pbHit = true;
						if (pHit)
	...
	if (pMC)
	{
		...
		if (/*pActor->IsPlayer() && */(HasFireHelper() && ShootFromHelper(pos, probableHit)))
		{
			...
			if (pActor)
			{
				...
				ICharacterInstance* pCharacter(pEntity ? pEntity->GetCharacter(0) : nullptr);
...
void CSingle::SetupEmitters(bool attach)
{
	if (attach)
	{
		...
		if (!m_muzzleflash.effect[0].empty())
			m_mfIds[m_barrelId].mfId[0] = m_pWeapon->AttachEffect(CItem::eIGS_FirstPerson, -1, true, m_muzzleflash.effect[0].c_str(), m_muzzleflash.helper[0].c_str(), offset, Vec3Constants<float>::fVec3_OneY, 1.0f, false);
		if (!m_muzzleflash.effect[1].empty())
			m_mfIds[m_barrelId].mfId[1] = m_pWeapon->AttachEffect(CItem::eIGS_ThirdPerson, -1, true, m_muzzleflash.effect[1].c_str(), m_muzzleflash.helper[1].c_str(), offset, Vec3Constants<float>::fVec3_OneY, 1.0f, false);
	}
	else
	{
	...
	if (attach)
	{
		...
		if (light && m_muzzleflash.light_radius[id] != 0.f)
		{
			if (!m_mflightId[id] && !m_muzzleflash.light_helper[id].empty())
				m_mflightId[id] = m_pWeapon->AttachLight(slot, 0, true, m_muzzleflash.light_radius[id], m_muzzleflash.light_color[id], 1.0f, nullptr, 0, m_muzzleflash.light_helper[id].c_str());
...
void CSingle::SpinUpEffect(bool attach)
{
	...
	if (attach)
	{
		...
		if (!m_spinup.effect[0].empty() || !m_spinup.effect[1].empty())
		{
			...
			m_suId = m_pWeapon->AttachEffect(slot, 0, true, m_spinup.effect[id].c_str(), m_spinup.helper[id].c_str(), Vec3Constants<float>::fVec3_Zero, Vec3Constants<float>::fVec3_OneY, 1.0f, false);

			//m_sulightId = m_pWeapon->AttachLight(slot, 0, true, m_spinup.light_radius[id], m_spinup.light_color[id], Vec3(1,1,1), 0, 0,
			m_sulightId = m_pWeapon->AttachLight(slot, 0, true, m_spinup.light_radius[id], m_spinup.light_color[id], 1, nullptr, 0, m_spinup.light_helper[id].c_str());
		}

		m_suTimer = static_cast<uint>(m_spinup.time[id]);
	}
	else
	{
...
void CSingle::RejectEffect()
{
	...
	if (!m_reject.effect[id].empty())
	{
		...
		dir.normalize();
		m_pWeapon->SpawnEffect(slot, m_reject.effect[id].c_str(), m_reject.helper[id].c_str(), offset, dir, m_reject.scale[id]);
	}
}
...
float CSingle::GetSpread() const
{
	...
	{
		if ((stance == STANCE_CROUCH && pActor->GetPreviousStance() == STANCE_STAND) || (stance == STANCE_STAND && pActor->GetPreviousStance() == STANCE_CROUCH))
...
{
	CActor*    pActor = m_pWeapon->GetOwnerActor();
	IAIObject* pOwnerAI = (pActor && pActor->GetEntity()) ? pActor->GetEntity()->GetAI() : nullptr;
	...
	if (m_recoilparams.decay > 0.0f)
	{
		if (m_fired)
		{
			...
			if (pActor && m_pWeapon->ApplyActorRecoil())
			{
				...
				e.param = static_cast<void*>(&recoil_add);
...
void CSingle::RecoilImpulse(const Vec3& firingPos, const Vec3& firingDir)
{
	...
	if (pActor && pActor->IsPlayer())
	{
		if (pActor->InZeroG())
		{
			...
			CPlayer* plr = static_cast<CPlayer*>(pActor);
...
float CSingle::GetProjectileFiringAngle(float v, float g, float x, float y)
{
	...
	if (d >= 0)
	{
		...
		if (a - d > 0)
		{
			...
			angle = acos_tpl(x / (v * t));
			float y_test;
			y_test = -v * sin_tpl(angle) * t - g * powf(t, 2) / 2;
			if (fabsf(y - y_test) < 0.02f)
				return RAD2DEG(-angle);
			y_test = static_cast<float>(v * sin_tpl(angle) * t - g * pow(t, 2) / 2);
			if (fabsf(y - y_test) < 0.02f)
				return RAD2DEG(angle);
		}
		t = cry_sqrtf(2 * (a + d) / powf(g, 2));
		angle = acos_tpl(x / (v * t));
		float y_test = static_cast<float>(v * sin_tpl(angle) * t - g * pow(t, 2) / 2);
