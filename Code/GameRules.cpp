...
CGameRules::CGameRules()
	...
	m_pGameplayRecorder(nullptr),
	m_pSystem(nullptr),
	m_pActorSystem(nullptr),
	m_pEntitySystem(nullptr),
	m_pScriptSystem(nullptr),
	m_pMaterialManager(nullptr),
	m_onCollisionFunc(nullptr),
	m_pClientNetChannel(nullptr),
	m_teamIdGen(0),
	...
	m_pRadio(nullptr),
	m_pBattleDust(nullptr),
	m_pMPTutorial(nullptr),
	m_pVotingSystem(nullptr),
	m_ignoreEntityNextCollision(0),
	m_timeOfDayInitialized(false),
	m_explosionScreenFX(true),
	m_pShotValidator(nullptr) {}
...
CGameRules::~CGameRules()
{
	...
	m_pGameFramework->GetIGameRulesSystem()->SetCurrentGameRules(nullptr);
...
bool CGameRules::Init(IGameObject* pGameObject)
{
	...
	const SmartScriptTable affected(gEnv->pScriptSystem);
	m_scriptExplosionInfo->SetValue("AffectedEntities", affected);
	const SmartScriptTable affectedObstruction(gEnv->pScriptSystem);
	...
	if (gEnv->bClient)
	{
		...
		const bool b = GetGameObject()->CaptureActions(this);
		assert(b);
...
void CGameRules::PostInitClient(const int channelId)
{
	...
	{
		const int                time_left = g_pGame->GetCVars()->sv_votingTimeout - static_cast<int>(m_pVotingSystem->GetVotingTime().GetSeconds());
		const VotingStatusParams params(m_pVotingSystem->GetType(), time_left, m_pVotingSystem->GetEntityId(), m_pVotingSystem->GetSubject());
...
//------------------------------------------------------------------------
void CGameRules::Update(SEntityUpdateContext& ctx, const int updateSlot)
{
	if (updateSlot != 0)
		return;
	...
	const bool server = gEnv->bServer;
			...
			for (TFrozenEntities::const_iterator fit = m_frozen.begin(); fit != m_frozen.end(); fit = next)
			{
				...
				if ((gEnv->pTimer->GetFrameStartTime() - fit->second).GetMilliSeconds() >= 750)
				{
					...
					if (m_pGameFramework->GetIVehicleSystem()->GetVehicle(fit->first))
						unfreeze = true;
					else if (const IItem* pItem = m_pGameFramework->GetIItemSystem()->GetItem(fit->first))
...
void CGameRules::PostUpdate(float frameTime)
{
	if (m_pVotingSystem && m_pVotingSystem->IsInProgress())
	{
		const int need_votes = static_cast<int>(ceilf(GetPlayerCount(false) * g_pGame->GetCVars()->sv_votingRatio));
		if (need_votes && m_pVotingSystem->GetNumVotes() >= need_votes)
			EndVoting(true);
		if (const int team = m_pVotingSystem->GetTeam())
		{
			const int team_votes = static_cast<int>(ceilf(GetTeamPlayerCount(team, false) * g_pGame->GetCVars()->sv_votingRatio));
...
//------------------------------------------------------------------------
void CGameRules::OnDisconnect(const EDisconnectionCause cause, const char* desc)
{
	m_pClientNetChannel = nullptr;
	const int icause    = cause;
...
//------------------------------------------------------------------------
bool CGameRules::OnClientConnect(const int channelId, const bool isReset)
{
	//TheOtherSide
	char buffer[256] = {};
	...
	const CActor* pActor = GetActorByChannelId(channelId);
	if (pActor)
	{
		...
		m_pGameplayRecorder->Event(pActor->GetEntity(), GameplayEvent(eGE_Connected, nullptr, m_pGameFramework->IsChannelOnHold(channelId) ? 1.0f : 0.0f, status));
...
//------------------------------------------------------------------------
void CGameRules::OnClientDisconnect(const int channelId, EDisconnectionCause cause, const char* desc, const bool keepClient)
{
	if (m_pShotValidator)
	...
	const std::vector<int>::iterator channelit = std::find(m_channelIds.begin(), m_channelIds.end(), channelId);
...
//------------------------------------------------------------------------
bool CGameRules::OnClientEnteredGame(const int channelId, const bool isReset)
{
	const CActor* pActor = GetActorByChannelId(channelId);
	if (!pActor)
		return false;
	...
	const int     loadingSaveGame = m_pGameFramework->IsLoadingSaveGame() ? 1 : 0;
...
//------------------------------------------------------------------------
void CGameRules::OnItemDropped(const EntityId itemId, const EntityId actorId)
{
	const ScriptHandle itemIdHandle(itemId);
	const ScriptHandle actorIdHandle(actorId);
	CallScript(m_serverStateScript, "OnItemDropped", itemIdHandle, actorIdHandle);
}

//------------------------------------------------------------------------
void CGameRules::OnItemPickedUp(const EntityId itemId, const EntityId actorId)
{
	const ScriptHandle itemIdHandle(itemId);
	const ScriptHandle actorIdHandle(actorId);
...
//------------------------------------------------------------------------
void CGameRules::OnTextMessage(const ETextMessageType type, const char* msg, const char* p0, const char* p1, const char* p2, const char* p3)
{
	switch (type)
	{
	...
	break;
	case eTextMessageError:SAFE_HUD_FUNC(DisplayFlashMessage(msg, 1, ColorF(0.85f,0,0), p0!=0, p0, p1, p2, p3));
		break;
	case eTextMessageInfo:SAFE_HUD_FUNC(DisplayFlashMessage(msg, 1, ColorF(1,1,1), p0!=0, p0, p1, p2, p3));
		break;
	case eTextMessageCenter:SAFE_HUD_FUNC(DisplayFlashMessage(msg, 2, ColorF(1,1,1), p0!=0, p0, p1, p2, p3));
		break;
	case eTextMessageBig:SAFE_HUD_FUNC(DisplayGameStartMessage(msg, p0, p1, p2));
		break;
	}
}

//------------------------------------------------------------------------
void CGameRules::OnChatMessage(EChatMessageType type, const EntityId sourceId, EntityId targetId, const char* msg, const bool teamChatOnly)
{
...
//------------------------------------------------------------------------
void CGameRules::OnRevive(CActor* pActor, const Vec3& pos, const Quat& rot, const int teamId)
{
	if (g_pGame->GetHUD())
		g_pGame->GetHUD()->ActorRevive(pActor);

	const ScriptHandle handle(pActor->GetEntityId());
	const Vec3         rotVec = Vec3(Ang3(rot));
	CallScript(m_clientScript, "OnRevive", handle, pos, rotVec, teamId);
}

//------------------------------------------------------------------------
void CGameRules::OnKill(CActor* pActor, const EntityId shooterId, const char* weaponClassName, int damage, int material, int hit_type)
{
...
//------------------------------------------------------------------------
void CGameRules::OnReviveInVehicle(CActor* pActor, EntityId vehicleId, const int seatId, const int teamId)
{
	const SGameObjectEvent evt(eCGE_ActorRevive, eGOEF_ToAll, IGameObjectSystem::InvalidExtensionID, pActor);
	SAFE_HUD_FUNC(HandleEvent(evt));

	const ScriptHandle handle(pActor->GetEntityId());
	const ScriptHandle vhandle(pActor->GetEntityId());
	CallScript(m_clientScript, "OnReviveInVehicle", handle, vhandle, seatId, teamId);
}

//------------------------------------------------------------------------
void CGameRules::OnVehicleDestroyed(const EntityId id, const EntityId shooterId)
{
...
//------------------------------------------------------------------------
void CGameRules::OnVehicleSubmerged(const EntityId id, const float ratio)
{
	RemoveSpawnGroup(id);
...
//------------------------------------------------------------------------
void CGameRules::AddTaggedEntity(const EntityId shooter, const EntityId targetId, const bool temporary)
{
	if (!gEnv->bServer) // server sends to all clients
		return;

	const EntityParams params(targetId);
	if (GetTeamCount() > 1)
	{
		const EntityId                         teamId = GetTeam(shooter);
		const TPlayerTeamIdMap::const_iterator tit    = m_playerteams.find(teamId);
	...
	const ScriptHandle shooterHandle(shooter);
	const ScriptHandle targetHandle(targetId);
	CallScript(m_serverScript, "OnAddTaggedEntity", shooterHandle, targetHandle);
}

//------------------------------------------------------------------------
void CGameRules::OnKillMessage(const EntityId targetId, const EntityId shooterId, const char* weaponClassName, float damage, const int material, const int hit_type)
{
	if (const EntityId client_id = g_pGame->GetIGameFramework()->GetClientActor() ? g_pGame->GetIGameFramework()->GetClientActor()->GetEntityId() : 0)
	{
...
//------------------------------------------------------------------------
CActor* CGameRules::SpawnPlayer(const int channelId, const char* name, const char* className, const Vec3& pos, const Ang3& angles)
{
	if (!gEnv->bServer)
		return nullptr;
...
//------------------------------------------------------------------------
CActor* CGameRules::ChangePlayerClass(const int channelId, const char* className)
{
	if (!gEnv->bServer)
		return nullptr;
	...
	const EntityId oldEntityId = pOldActor->GetEntityId();
	const string   oldName     = pOldActor->GetEntity()->GetName();
	const Ang3     oldAngles   = pOldActor->GetAngles();
	const Vec3     oldPos      = pOldActor->GetEntity()->GetWorldPos();
...
void CGameRules::RevivePlayerInVehicle(CTOSActor* pActor, const EntityId vehicleId, const int seatId, const int teamId/* =0 */, const bool clearInventory/* =true */)
{
	...
	if (seatId < 0)
	{
		...
		{
			const Ang3 angles = pSpawnVehicle->GetEntity()->GetWorldAngles(); // face same direction as vehicle.
	...
	pActor->GetGameObject()->InvokeRMI(CActor::ClReviveInVehicle(), CActor::ReviveInVehicleParams(vehicleId, seatId, teamId), eRMI_ToAllClients | eRMI_NoLocalCalls);
...
{
	const string             fixed = VerifyName(name, pActor->GetEntity());
	const RenameEntityParams params(pActor->GetEntityId(), fixed.c_str());
...
bool CGameRules::IsNameTaken(const char* name, IEntity* pEntity)
{
	...
	{
		const CActor* pActor = GetActorByChannelId(*it);
...
void CGameRules::KillPlayer(CTOSActor *pActor, const bool dropItem, const bool ragdoll, const EntityId shooterId, const EntityId weaponId, const float damage, const int material, const int hit_type, const Vec3& impulse)
{
	...
	const EntityId itemId     = pInventory ? pInventory->GetCurrentItem() : 0;
	...
	{
		const char* weaponClassName = pEntity->GetClass()->GetName();
	...
	pActor->NetKill(shooterId, weaponClassId, static_cast<int>(damage), material, hit_type, shooterhealth);

	pActor->GetGameObject()->InvokeRMI(CActor::ClKill(), CActor::KillParams(shooterId, weaponClassId, damage, material, hit_type, impulse, shooterhealth), eRMI_ToAllClients | eRMI_NoLocalCalls);

	m_pGameplayRecorder->Event(pActor->GetEntity(), GameplayEvent(eGE_Death));
	if (shooterId && shooterId != pActor->GetEntityId())
		if (const IActor* pShooter = m_pGameFramework->GetIActorSystem()->GetActor(shooterId))
			m_pGameplayRecorder->Event(pShooter->GetEntity(), GameplayEvent(eGE_Kill, nullptr, 0, (void*)weaponId));
}

//------------------------------------------------------------------------
void CGameRules::MovePlayer(CActor* pActor, const Vec3& pos, const Ang3& angles)
{
	const CActor::MoveParams params(pos, Quat(angles));
...
//------------------------------------------------------------------------
void CGameRules::ChangeSpectatorMode(CActor* pActor, const uint8 mode, const EntityId targetId, const bool resetAll)
{
	if (pActor->GetSpectatorMode() == mode && mode != CActor::eASM_Follow)
		return;

	const SpectatorModeParams params(pActor->GetEntityId(), mode, targetId, resetAll);
...
//------------------------------------------------------------------------
void CGameRules::RequestNextSpectatorTarget(CActor* pActor, const int change)
{
	...
	{
		const ScriptHandle playerId(pActor->GetEntityId());
...
void CGameRules::ChangeTeam(IActor* pActor, const char* teamName)
{
	...
	const int teamId = GetTeamId(teamName);
...
//------------------------------------------------------------------------
int CGameRules::GetPlayerCount(const bool inGame) const
{
	if (!inGame)
		return static_cast<int>(m_channelIds.size());
...
//------------------------------------------------------------------------
int CGameRules::GetSpectatorCount(const bool inGame) const
{
	int count = 0;
	for (std::vector<int>::const_iterator it = m_channelIds.begin(); it != m_channelIds.end(); ++it)
	{
...
//------------------------------------------------------------------------
EntityId CGameRules::GetPlayer(const int idx)
{
	if (idx < 0 || idx >= m_channelIds.size())
		return 0;

	const CActor* pActor = GetActorByChannelId(m_channelIds[idx]);
...
//------------------------------------------------------------------------
bool CGameRules::IsPlayerInGame(const EntityId playerId) const
{
	const INetChannel* pNetChannel = g_pGame->GetIGameFramework()->GetNetChannel(GetChannelId(playerId));
...
//------------------------------------------------------------------------
bool CGameRules::IsPlayerActivelyPlaying(const EntityId playerId, const bool mustBeAlive) const
{
	if (!gEnv->bMultiplayer)
		return true;
	...
	if (GetTeamCount() > 1)
	{
		...
		const CActor* pActor = reinterpret_cast<CActor*>(g_pGame->GetIGameFramework()->GetIActorSystem()->GetActor(playerId));
		if (!pActor)
			return false;
...
//------------------------------------------------------------------------
bool CGameRules::IsChannelInGame(const int channelId) const
{
	const INetChannel* pNetChannel = g_pGame->GetIGameFramework()->GetNetChannel(channelId);
	if (pNetChannel && pNetChannel->GetContextViewState() >= eCVS_InGame)
		return true;
	return false;
}
...
//------------------------------------------------------------------------
void CGameRules::StartVoting(CActor* pActor, const EVotingState t, const EntityId id, const char* param)
{
	const StartVotingParams params(t, id, param);
	EntityId          entityId = 0;
	if (pActor)
	...
	if (gEnv->bServer)
	{
		...
		CTimeValue       st;
		const CTimeValue curr_time = gEnv->pTimer->GetFrameStartTime();
		...
		if (!m_pVotingSystem->GetCooldownTime(entityId, st) || (curr_time - st).GetSeconds() > g_pGame->GetCVars()->sv_votingCooldown)
		{
			if (m_pVotingSystem->StartVoting(entityId, curr_time, t, id, param, GetTeam(id)))
			{
				...
				const VotingStatusParams st_param(t, g_pGame->GetCVars()->sv_votingTimeout, id, param);
...
//------------------------------------------------------------------------
void CGameRules::Vote(CActor* pActor, const bool yes)
{
	if (!pActor)
		return;
...
//------------------------------------------------------------------------
void CGameRules::EndVoting(const bool success)
{
	...
	if (success)
	{
		...
		case eVS_kick:
		{
			const int ch_id = GetChannelId(m_pVotingSystem->GetEntityId());
	...
	const VotingStatusParams params(eVS_none, 0, GetEntityId(), "");
...
{
	const TTeamIdMap::iterator it = m_teams.find(CONST_TEMP_STRING(name));
	if (it != m_teams.end())
		return it->second;
...
//------------------------------------------------------------------------
void CGameRules::RemoveTeam(const int teamId)
{
	const TTeamIdMap::iterator it = m_teams.find(CONST_TEMP_STRING(GetTeamName(teamId)));
	if (it == m_teams.end())
		return;
...
//------------------------------------------------------------------------
const char* CGameRules::GetTeamName(const int teamId) const
{
...
{
	const TTeamIdMap::const_iterator it = m_teams.find(CONST_TEMP_STRING(name));
	if (it != m_teams.end())
		return it->second;
...
//------------------------------------------------------------------------
int CGameRules::GetTeamPlayerCount(const int teamId, const bool inGame, const bool isActive, const EntityId skip) const
{
	if (!inGame)
	{
		const TPlayerTeamIdMap::const_iterator it = m_playerteams.find(teamId);
...
//------------------------------------------------------------------------
int CGameRules::GetTeamChannelCount(const int teamId, const bool inGame) const
{
	int count = 0;
...
//------------------------------------------------------------------------
EntityId CGameRules::GetTeamActivePlayer(const int teamId, const int idx) const
{
	const TPlayerTeamIdMap::const_iterator it = m_playerteams.find(teamId);
	if (it == m_playerteams.end())
		return 0;
...
//------------------------------------------------------------------------
EntityId CGameRules::GetTeamPlayer(const int teamId, const int idx) const
{
	const TPlayerTeamIdMap::const_iterator it = m_playerteams.find(teamId);
	if (it != m_playerteams.end())
...
//------------------------------------------------------------------------
void CGameRules::GetTeamPlayers(const int teamId, TPlayers& players)
{
	players.resize(0);
	const TPlayerTeamIdMap::const_iterator it = m_playerteams.find(teamId);
	if (it != m_playerteams.end())
...
//------------------------------------------------------------------------
void CGameRules::SetTeam(int teamId, EntityId entityId)
{
	if (!gEnv->bServer)
	{
		assert(0);
	...
	const int oldTeam = GetTeam(entityId);
	if (oldTeam == teamId)
		return;

	const TEntityTeamIdMap::iterator it = m_entityteams.find(entityId);
	...
	if (isplayer && oldTeam)
	{
		const TPlayerTeamIdMap::iterator pit = m_playerteams.find(oldTeam);
	...
	if (teamId)
	{
		...
		if (isplayer)
		{
			const TPlayerTeamIdMap::iterator pit = m_playerteams.find(teamId);
			assert(pit!=m_playerteams.end());
	...
	if (isplayer)
	{
		...
		const TChannelTeamIdMap::iterator it = m_channelteams.find(channelId);
...
//------------------------------------------------------------------------
int CGameRules::GetTeam(const EntityId entityId) const
{
	const TEntityTeamIdMap::const_iterator it = m_entityteams.find(entityId);
	if (it != m_entityteams.end())
		return it->second;
...
//------------------------------------------------------------------------
int CGameRules::GetChannelTeam(const int channelId) const
{
	const TChannelTeamIdMap::const_iterator it = m_channelteams.find(channelId);
	if (it != m_channelteams.end())
		return it->second;
...
//------------------------------------------------------------------------
void CGameRules::AddObjective(int teamId, const char* objective, const int status, const EntityId entityId)
{
...
//------------------------------------------------------------------------
void CGameRules::SetObjectiveStatus(const int teamId, const char* objective, const int status)
{
	if (TObjective* pObjective = GetObjective(teamId, objective))
		pObjective->status = status;

	if (gEnv->bServer) GAMERULES_INVOKE_ON_TEAM(teamId, ClSetObjectiveStatus(), SetObjectiveStatusParams(objective, status))
}

//------------------------------------------------------------------------
void CGameRules::SetObjectiveEntity(const int teamId, const char* objective, const EntityId entityId)
{
	if (TObjective* pObjective = GetObjective(teamId, objective))
		pObjective->entityId = entityId;

	if (gEnv->bServer) GAMERULES_INVOKE_ON_TEAM(teamId, ClSetObjectiveEntity(), SetObjectiveEntityParams(objective, entityId))
}

//------------------------------------------------------------------------
void CGameRules::RemoveObjective(const int teamId, const char* objective)
{
	if (TObjectiveMap* pObjectives = GetTeamObjectives(teamId))
	{
		const TObjectiveMap::iterator it = pObjectives->find(CONST_TEMP_STRING(objective));
...
//------------------------------------------------------------------------
CGameRules::TObjectiveMap* CGameRules::GetTeamObjectives(const int teamId)
{
	const TTeamObjectiveMap::iterator it = m_objectives.find(teamId);
...
//------------------------------------------------------------------------
CGameRules::TObjective* CGameRules::GetObjective(const int teamId, const char* objective)
{
	if (TObjectiveMap* pObjectives = GetTeamObjectives(teamId))
	{
		const TObjectiveMap::iterator it = pObjectives->find(CONST_TEMP_STRING(objective));
...
//------------------------------------------------------------------------
void CGameRules::UpdateObjectivesForPlayer(const int channelId, const int teamId)
{
...
//------------------------------------------------------------------------
bool CGameRules::IsFrozen(const EntityId entityId) const
{
	if (m_frozen.find(entityId) != m_frozen.end())
		return true;

	const IEntity* pEntity = m_pEntitySystem->GetEntity(entityId);
	if (!pEntity)
		return false;
...
void CGameRules::FreezeEntity(EntityId entityId, bool freeze, bool vapor, bool force)
{
	...
	if (freeze && gEnv->bServer)
	{
		...
		{
			IEntity* pObject = nullptr;
			if (gEnv->pAISystem->SmartObjectEvent("DontFreeze", pEntity, pObject, nullptr))
				return;
		...
		HSCRIPTFUNCTION pfnGetFrozenAmount = nullptr;
...
//------------------------------------------------------------------------
void CGameRules::ShatterEntity(const EntityId entityId, const Vec3& pos, const Vec3& impulse)
{
	IEntity* pEntity = m_pEntitySystem->GetEntity(entityId);
	if (!pEntity)
		return;
	...
	psj.idx = 0;
	if (pEntity->GetFlags() & ENTITY_FLAG_MODIFIED_BY_PHYSICS || pEntity->GetPhysics() && pEntity->GetPhysics()->GetParams(&psj))
		return;
	...
	if (pScriptTable)
	{
		...
		HSCRIPTFUNCTION pfnCanShatter = nullptr;
	...
	if (pGameObject)
	{
		const SGameObjectEvent event(eCGE_PreShatter, eGOEF_ToAll, IGameObjectSystem::InvalidExtensionID, nullptr);
		pGameObject->SendEvent(event);
	}
	else
	{
	...
	if (pScriptTable)
	{
		HSCRIPTFUNCTION pfnGetFrozenSlot = nullptr;
	...
	*/
	...
	if (pGameObject)
	{
		const SGameObjectEvent event(eCGE_PostShatter, eGOEF_ToAll, IGameObjectSystem::InvalidExtensionID, nullptr);
		pGameObject->SendEvent(event);
	}
	...
	// shatter children
	const int n = pEntity->GetChildCount();
	for (int i = n - 1; i >= 0; --i)
	{
		if (const IEntity* pChild = pEntity->GetChild(i))
			ShatterEntity(pChild->GetId(), pos, impulse);
	}

	FreezeEntity(entityId, false, false);

	const IActor*    pActor   = m_pGameFramework->GetIActorSystem()->GetActor(entityId);
	const bool isPlayer = (pActor && pActor->IsPlayer());
...
struct compare_spawns
{
	...
	{
		const int lhsT = g_pGame->GetGameRules()->GetTeam(lhs);
		const int rhsT = g_pGame->GetGameRules()->GetTeam(rhs);
		if (lhsT == rhsT)
		{
			const EntityId lhsG = g_pGame->GetGameRules()->GetSpawnLocationGroup(lhs);
			const EntityId rhsG = g_pGame->GetGameRules()->GetSpawnLocationGroup(rhs);
			if (lhsG == rhsG)
			{
				const IEntity* pLhs = gEnv->pEntitySystem->GetEntity(lhs);
				const IEntity* pRhs = gEnv->pEntitySystem->GetEntity(rhs);
...
//------------------------------------------------------------------------
void CGameRules::AddSpawnLocation(const EntityId location)
{
...
//------------------------------------------------------------------------
void CGameRules::RemoveSpawnLocation(const EntityId id)
{
...
{
	return static_cast<int>(m_spawnLocations.size());
}

//------------------------------------------------------------------------
EntityId CGameRules::GetSpawnLocation(const int idx) const
{
	if (idx >= 0 && idx < static_cast<int>(m_spawnLocations.size()))
...
//------------------------------------------------------------------------
bool CGameRules::IsSpawnLocationSafe(const EntityId playerId, const EntityId spawnLocationId, const float safeDistance, const float zoffset) const
{
	const IEntity* pSpawn = gEnv->pEntitySystem->GetEntity(spawnLocationId);
	if (!pSpawn)
		return false;
	...
	const Vec3            pos2check(pSpawn->GetWorldPos());
	const float           l(safeDistance * 1.5f);
	...
	if (zoffset <= 0.0001f)
		...
		{
			const EntityId entityId = query.pEntities[i]->GetId();
...
//------------------------------------------------------------------------
bool CGameRules::IsSpawnLocationFarEnough(const EntityId spawnLocationId, const float minDistance, const Vec3& testPosition) const
{
	if (minDistance <= 0.1f)
		return true;
...
//------------------------------------------------------------------------
bool CGameRules::TestSpawnLocationWithEnvironment(const EntityId spawnLocationId, const EntityId playerId, const float offset, const float height) const
{
	if (!spawnLocationId)
		return false;
	...
	IPhysicalEntity* pPlayerPhysics = nullptr;

	const IEntity* pPlayer = gEnv->pEntitySystem->GetEntity(playerId);
	if (pPlayer)
	...
	const float   dst      = gEnv->pPhysicalWorld->PrimitiveWorldIntersection(sphere.type, &sphere, end - sphere.center, ent_static | ent_terrain | ent_rigid | ent_sleeping_rigid | ent_living, &pContact, 0, (geom_colltype_player << rwi_colltype_bit) | rwi_stop_at_pierceable, nullptr, nullptr, 0, &pPlayerPhysics, pPlayerPhysics ? 1 : 0);
...
//------------------------------------------------------------------------
EntityId CGameRules::GetSpawnLocation(const EntityId playerId, const bool ignoreTeam, const bool includeNeutral, const EntityId groupId, const float minDistToDeath, const Vec3& deathPos, float* pZOffset, const EntityId skipId) const
{
	...
	if (groupId)
	{
		const TSpawnGroupMap::const_iterator it = m_spawnGroups.find(groupId);
...
float CGameRules::GetMinEnemyDist() const
{
	...
	float minX  = minY;
	for (int count = GetSpawnLocationCount() - 1; count >= 0; --count)
	{
...
EntityId CGameRules::GetSpawnLocationTeamFirst() const
{
	...
	for (TSpawnLocations::const_iterator it = m_spawnLocations.begin(); it != m_spawnLocations.end(); ++it)
	{
		...
		//			continue;
		const IEntity* pSpawn(gEnv->pEntitySystem->GetEntity(spawnId));
		const float    sqrDist = (avrgPos - pSpawn->GetWorldPos()).GetLengthSquared();
...
//------------------------------------------------------------------------
EntityId CGameRules::GetSpawnLocationTeam(const EntityId playerId, const Vec3& deathPos) const
{
	FUNCTION_PROFILER(GetISystem(), PROFILE_GAME);
	...
	if (playerTeamSize < 2)
	{
		...
		for (TSpawnLocations::const_iterator it = m_spawnLocations.begin(); it != m_spawnLocations.end(); ++it)
		{
			...
			const float    deathDistSqr((spawnPos - deathPos).GetLengthSquared());
			if (deathDistSqr < minDistToDeathSqr) // too close to death position
				continue;
			if (!IsSpawnLocationSafe(playerId, spawnId, safeDistance, 0.f))
				continue;
			const float closeDist(GetClosestTeamMateDistSqr(enemyTeamId, spawnPos));
	...
	for (TSpawnLocations::const_iterator it = m_spawnLocations.begin(); it != m_spawnLocations.end(); ++it)
	{
		...
		if (!IsSpawnLocationSafe(playerId, spawnId, safeDistance, 0.f))
			continue;
		const float closestEnemyDistSqr(GetClosestTeamMateDistSqr(enemyTeamId, spawnPos));
		if (closestEnemyDistSqr < enemyRejectDistSqr)
			continue;
		const float closeDist(GetClosestTeamMateDistSqr(playerTeamId, spawnPos, playerId));
...
//------------------------------------------------------------------------
int CGameRules::GetEnemyTeamId(const int myTeamId) const
{
	for (TPlayerTeamIdMap::const_iterator it = m_playerteams.begin(); it != m_playerteams.end(); ++it)
	{
...
float CGameRules::GetClosestPlayerDistSqr(const EntityId spawnLocationId, const EntityId skipId) const
{
	...
	TPlayers    players;
	GetPlayers(players);
	if (players.empty() || players[0] == skipId && players.size() == 1)
		return closestDist;
	for (TPlayers::iterator it = players.begin(); it != players.end(); ++it)
	{
		...
		const float    squareDist = (pos - pOther->GetWorldPos()).GetLengthSquared();
...
//------------------------------------------------------------------------
float CGameRules::GetClosestTeamMateDistSqr(const int teamId, const Vec3& pos, const EntityId skipId) const
{
	FUNCTION_PROFILER(GetISystem(), PROFILE_GAME);
	float closestDist(std::numeric_limits<float>::max());
	...
	while (teamMateId = GetTeamActivePlayer(teamId, idx++))
	{
		...
		const float    squareDist = (pos - pTeamMate->GetWorldPos()).GetLengthSquared();
		if (closestDist < squareDist)
			continue;
		closestDist = squareDist;
	}
...
//------------------------------------------------------------------------
EntityId CGameRules::GetFirstSpawnLocation(const int teamId, const EntityId groupId) const
{
...
//------------------------------------------------------------------------
void CGameRules::AddSpawnLocationToSpawnGroup(const EntityId groupId, const EntityId location)
{
	const TSpawnGroupMap::iterator it = m_spawnGroups.find(groupId);
	if (it == m_spawnGroups.end())
		return;
...
//------------------------------------------------------------------------
void CGameRules::RemoveSpawnLocationFromSpawnGroup(const EntityId groupId, const EntityId location)
{
	const TSpawnGroupMap::iterator it = m_spawnGroups.find(groupId);
	if (it == m_spawnGroups.end())
		return;
...
//------------------------------------------------------------------------
void CGameRules::RemoveSpawnGroup(const EntityId groupId)
{
	const TSpawnGroupMap::iterator it = m_spawnGroups.find(groupId);
	if (it != m_spawnGroups.end())
...
//------------------------------------------------------------------------
EntityId CGameRules::GetSpawnLocationGroup(const EntityId spawnId) const
{
...
{
	return static_cast<int>(m_spawnGroups.size());
}

//------------------------------------------------------------------------
EntityId CGameRules::GetSpawnGroup(const int idx) const
{
	if (idx >= 0 && idx < static_cast<int>(m_spawnGroups.size()))
	{
...
//------------------------------------------------------------------------
bool CGameRules::IsSpawnGroup(const EntityId id) const
{
	const TSpawnGroupMap::const_iterator it = m_spawnGroups.find(id);
	return it != m_spawnGroups.end();
}

//------------------------------------------------------------------------
void CGameRules::RequestSpawnGroup(const EntityId spawnGroupId)
{
	CallScript(m_script, "RequestSpawnGroup", ScriptHandle(spawnGroupId));
}

//------------------------------------------------------------------------
void CGameRules::SetPlayerSpawnGroup(const EntityId playerId, const EntityId spawnGroupId)
{
...
EntityId CGameRules::GetPlayerSpawnGroup(CActor* pActor)
{
	...
	return static_cast<EntityId>(ret.n);
}

//------------------------------------------------------------------------
void CGameRules::SetTeamDefaultSpawnGroup(int teamId, EntityId spawnGroupId)
{
	const TTeamIdEntityIdMap::iterator it = m_teamdefaultspawns.find(teamId);
...
//------------------------------------------------------------------------
EntityId CGameRules::GetTeamDefaultSpawnGroup(const int teamId)
{
	const TTeamIdEntityIdMap::iterator it = m_teamdefaultspawns.find(teamId);
	if (it != m_teamdefaultspawns.end())
		return it->second;
...
//------------------------------------------------------------------------
void CGameRules::CheckSpawnGroupValidity(const EntityId spawnGroupId)
{
	const bool exists = spawnGroupId && (m_spawnGroups.find(spawnGroupId) != m_spawnGroups.end()) && (gEnv->pEntitySystem->GetEntity(spawnGroupId) != nullptr);
	const bool valid  = exists && GetTeam(spawnGroupId) != 0;
...
//------------------------------------------------------------------------
void CGameRules::AddSpectatorLocation(const EntityId location)
{
	stl::push_back_unique(m_spectatorLocations, location);
}

//------------------------------------------------------------------------
void CGameRules::RemoveSpectatorLocation(const EntityId id)
{
...
{
	return static_cast<int>(m_spectatorLocations.size());
}

//------------------------------------------------------------------------
EntityId CGameRules::GetSpectatorLocation(const int idx) const
{
...
{
	const int idx = Random(GetSpectatorLocationCount());
...
//------------------------------------------------------------------------
void CGameRules::UpdateMinimap(const float frameTime)
{
...
//------------------------------------------------------------------------
void CGameRules::AddMinimapEntity(const EntityId entityId, const int type, const float lifetime)
{
	const TMinimap::iterator it = std::find(m_minimap.begin(), m_minimap.end(), SMinimapEntity(entityId, 0, 0.0f));
	if (it != m_minimap.end())
	{
		if (type > it->type)
			it->type = type;
	...
	if (gEnv->bServer)
		GetGameObject()->InvokeRMIWithDependentObject(ClAddMinimapEntity(), AddMinimapEntityParams(entityId, lifetime, type), eRMI_ToAllClients | eRMI_NoLocalCalls, entityId);
}

//------------------------------------------------------------------------
void CGameRules::RemoveMinimapEntity(const EntityId entityId)
{
...
{
	if (const int id = GetHitMaterialId(materialName))
		return id;
...
int CGameRules::GetHitMaterialId(const char* materialName) const
{
	...
	const int id = pSurfaceType->GetId();
...
//------------------------------------------------------------------------
int CGameRules::GetHitMaterialIdFromSurfaceId(const int surfaceId) const
{
	for (THitMaterialMap::const_iterator it = m_hitMaterials.begin(); it != m_hitMaterials.end(); ++it)
	{
...
//------------------------------------------------------------------------
ISurfaceType* CGameRules::GetHitMaterial(const int id) const
{
	const THitMaterialMap::const_iterator it = m_hitMaterials.find(id);
...
{
	if (const int id = GetHitTypeId(type))
		return id;
...
//------------------------------------------------------------------------
const char* CGameRules::GetHitType(const int id) const
{
	const THitTypeMap::const_iterator it = m_hitTypes.find(id);
...
//------------------------------------------------------------------------
void CGameRules::SendTextMessage(const ETextMessageType type, const char* msg, const unsigned int to, const int channelId, const char* p0, const char* p1, const char* p2, const char* p3)
{
	GetGameObject()->InvokeRMI(ClTextMessage(), TextMessageParams(type, msg, p0, p1, p2, p3), to, channelId);
}

//------------------------------------------------------------------------
bool CGameRules::CanReceiveChatMessage(EChatMessageType type, const EntityId sourceId, const EntityId targetId) const
{
	if (sourceId == targetId)
		return true;

	const bool sspec = !IsPlayerActivelyPlaying(sourceId);
	const bool sdead = IsDead(sourceId);

	const bool tspec = !IsPlayerActivelyPlaying(targetId);
	const bool tdead = IsDead(targetId);
...
//------------------------------------------------------------------------
void CGameRules::ChatLog(const EChatMessageType type, const EntityId sourceId, const EntityId targetId, const char* msg)
{
	const IEntity* pSource    = gEnv->pEntitySystem->GetEntity(sourceId);
	...
	const int      teamId     = GetTeam(sourceId);
...
//------------------------------------------------------------------------
void CGameRules::SendChatMessage(const EChatMessageType type, const EntityId sourceId, const EntityId targetId, const char* msg)
{
	const ChatMessageParams params(type, sourceId, targetId, msg, (type == eChatToTeam) ? true : false);
	...
	if (gEnv->bServer)
		...
		case eChatToAll:
		{
			const std::vector<int>::const_iterator begin = m_channelIds.begin();
			const std::vector<int>::const_iterator end   = m_channelIds.end();

			for (std::vector<int>::const_iterator it = begin; it != end; ++it)
			{
				if (const CActor* pActor = GetActorByChannelId(*it))
		...
		case eChatToTeam:
		{
			...
			if (teamId)
			{
				...
				if (tit != m_playerteams.end())
				{
					const TPlayers::const_iterator begin = tit->second.begin();
					const TPlayers::const_iterator end   = tit->second.end();
...
//------------------------------------------------------------------------
void CGameRules::ForbiddenAreaWarning(const bool active, const int timer, const EntityId targetId)
{
...
void CGameRules::ResetGameTime()
{
	...
	const float timeLimit = g_pGameCVars->g_timelimit;
	if (timeLimit > 0.0f)
...
//------------------------------------------------------------------------
void CGameRules::AddOvertime(const float overTime)
{
	if (overTime > 0.0f)
...
void CGameRules::ResetRoundTime()
{
	...
	const float roundTime = g_pGameCVars->g_roundtime;
	if (roundTime > 0.0f)
...
void CGameRules::ResetPreRoundTime()
{
	...
	const int preRoundTime = g_pGameCVars->g_preroundtime;
	if (preRoundTime > 0)
...
void CGameRules::ResetReviveCycleTime()
{
	...
	m_reviveCycleEndTime = m_pGameFramework->GetServerTime() + static_cast<float>(g_pGameCVars->g_revivetime);
...
//------------------------------------------------------------------------
void CGameRules::ResetGameStartTimer(const float time)
{
	if (!gEnv->bServer)
	{
...
bool CGameRules::OnCollision(const SGameCollision& event)
{
	...
	if (!gEnv->bMultiplayer)
	{
		const IEntity* pTarget = event.pCollision->iForeignData[1] == PHYS_FOREIGN_ID_ENTITY ? static_cast<IEntity*>(event.pCollision->pForeignData[1]) : nullptr;
		if (pTarget)
		{
	...
	const IEntityClass* pSrcClass = nullptr;
	if (event.pSrcEntity)
	{
	...
	const IEntityClass* pTrgClass = nullptr;
	if (event.pTrgEntity)
	{
		// filter out any projectile collisions
...
void CGameRules::CmdDebugSpawns(IConsoleCmdArgs* pArgs)
{
	...
	if (!pGameRules->m_spawnGroups.empty())
	{
		...
		{
			const IEntity* pEntity     = gEnv->pEntitySystem->GetEntity(sit->first);
			const int      groupTeamId = pGameRules->GetTeam(pEntity->GetId());
			const char*    Default     = "$5*DEFAULT*";
			CryLogAlways("Spawn Group: %s  (eid: %d %08x  team: %d) %s", pEntity->GetName(), pEntity->GetId(), pEntity->GetId(), groupTeamId, (sit->first == pGameRules->GetTeamDefaultSpawnGroup(groupTeamId)) ? Default : "");

			for (TSpawnLocations::const_iterator lit = sit->second.begin(); lit != sit->second.end(); ++lit)
			{
				const int            spawnTeamId = pGameRules->GetTeam(pEntity->GetId());
				const IEntity* pEntity     = gEnv->pEntitySystem->GetEntity(*lit);
				const char*    cs          = "";
	...
	{
		const IEntity* pEntity = gEnv->pEntitySystem->GetEntity(*lit);
		const Vec3     pos     = pEntity ? pEntity->GetWorldPos() : ZERO;
...
void CGameRules::CmdDebugMinimap(IConsoleCmdArgs* pArgs)
{
	...
	if (!pGameRules->m_minimap.empty())
	{
		...
		{
			const IEntity* pEntity = gEnv->pEntitySystem->GetEntity(it->entityId);
	...
	if (!pGameRules->m_entityteams.empty())
	{
		...
		for (TTeamIdMap::const_iterator tit = pGameRules->m_teams.begin(); tit != pGameRules->m_teams.end(); ++tit)
		{
			...
			for (TEntityTeamIdMap::const_iterator eit = pGameRules->m_entityteams.begin(); eit != pGameRules->m_entityteams.end(); ++eit)
			{
				...
				{
					const IEntity* pEntity = gEnv->pEntitySystem->GetEntity(eit->first);
	...
	if (!pGameRules->m_objectives.empty())
	{
		...
		for (TTeamIdMap::const_iterator tit = pGameRules->m_teams.begin(); tit != pGameRules->m_teams.end(); ++tit)
		{
			if (TObjectiveMap* pObjectives = pGameRules->GetTeamObjectives(tit->second))
				...
				{
					CryLogAlways("  -> Objective: %s  teamId: %d  status: %s  (eid: %d %08x)", it->first.c_str(), tit->second, status[CLAMP(it->second.status, 0, CHUDMissionObjective::LAST)], it->second.entityId, it->second.entityId);
				}
		}
	}
}
...
//------------------------------------------------------------------------
void CGameRules::CreateScriptHitInfo(const SmartScriptTable& scriptHitInfo, const HitInfo& hitInfo) const
{
	{
		...
		const IEntity* pWeapon     = m_pEntitySystem->GetEntity(hitInfo.weaponId);
		const IEntity* pProjectile = m_pEntitySystem->GetEntity(hitInfo.projectileId);

		hit.SetValue("projectile", pProjectile ? pProjectile->GetScriptTable() : static_cast<IScriptTable*>(nullptr));
		hit.SetValue("target", pTarget ? pTarget->GetScriptTable() : static_cast<IScriptTable*>(nullptr));
		hit.SetValue("shooter", pShooter ? pShooter->GetScriptTable() : static_cast<IScriptTable*>(nullptr));
		hit.SetValue("weapon", pWeapon ? pWeapon->GetScriptTable() : static_cast<IScriptTable*>(nullptr));
		...
		float assist = 0.0f;
		if (pShooter && ((g_pGameCVars->hit_assistSingleplayerEnabled && !gEnv->bMultiplayer) || (g_pGameCVars->hit_assistMultiplayerEnabled && gEnv->bMultiplayer)))
		{
...
//------------------------------------------------------------------------
void CGameRules::CreateScriptExplosionInfo(const SmartScriptTable& scriptExplosionInfo, const ExplosionInfo& explosionInfo)
{
	{
		const CScriptSetGetChain explosion(scriptExplosionInfo);
		...
		const IEntity* pShooter = m_pEntitySystem->GetEntity(explosionInfo.shooterId);
		const IEntity* pWeapon  = m_pEntitySystem->GetEntity(explosionInfo.weaponId);
		explosion.SetValue("shooter", pShooter ? pShooter->GetScriptTable() : static_cast<IScriptTable*>(nullptr));
		explosion.SetValue("weapon", pWeapon ? pWeapon->GetScriptTable() : static_cast<IScriptTable*>(nullptr));
...
void CGameRules::ShowScores(const bool show)
{
		...
		for (int i = 0; i < pExplosion->nAffectedEnts; ++i)
		{
			if (IEntity*          pEntity      = gEnv->pEntitySystem->GetEntityFromPhysics(pExplosion->pAffectedEnts[i]))
				if (IScriptTable* pEntityTable = pEntity->GetScriptTable())
				{
					...
					if (pEnt)
					{
						const float affected = gEnv->pPhysicalWorld->IsAffectedByExplosion(pEnt);
...
{
	const TExplosionAffectedEntities::iterator it = affectedEnts.find(pEntity);
...
//------------------------------------------------------------------------
void CGameRules::CommitAffectedEntitiesSet(const SmartScriptTable& scriptExplosionInfo, TExplosionAffectedEntities& affectedEnts)
{
	...
	if (scriptExplosionInfo->GetValue("AffectedEntities", affected) && scriptExplosionInfo->GetValue("AffectedEntitiesObstruction", affectedObstruction))
	{
...
//------------------------------------------------------------------------
void CGameRules::PrepCollision(const int src, const int trg, const SGameCollision& event, const IEntity* pTarget) const
{
	...
	if (pTarget)
	{
		...
		if (pTarget->GetPhysics())
			chain.SetValue("target_type", static_cast<int>(pTarget->GetPhysics()->GetType()));
...
//------------------------------------------------------------------------
void CGameRules::GameOver(const int localWinner, const int winnerTeam, const EntityId id)
{
...
//------------------------------------------------------------------------
void CGameRules::EndGameNear(const EntityId id)
{
	if (m_rulesListeners.empty() == false)
	{
		TGameRulesListenerVec::iterator iter = m_rulesListeners.begin();
...
//------------------------------------------------------------------------
bool CGameRules::HasEntityRespawnData(const EntityId entityId) const
{
	return m_respawndata.find(entityId) != m_respawndata.end();
}

//------------------------------------------------------------------------
void CGameRules::ScheduleEntityRespawn(EntityId entityId, const bool unique, const bool spatialcheck, const float timer)
{
	if (!gEnv->bServer || m_pGameFramework->IsEditing())
		return;
...
//------------------------------------------------------------------------
void CGameRules::UpdateEntitySchedules(const float frameTime)
{
	if (!gEnv->bServer || m_pGameFramework->IsEditing())
		return;
	...
	for (TEntityRespawnMap::iterator it = m_respawns.begin(); it != m_respawns.end(); it = next)
	{
		...
		if (respawn.unique)
		{
			const IEntity* pEntity = m_pEntitySystem->GetEntity(id);
			if (pEntity)
				continue;
		}
...
{
	geom_contact* pContact = nullptr;
	const float   dist     = gEnv->pPhysicalWorld->PrimitiveWorldIntersection(primitives::box::type, &obb, ZERO, ent_rigid | ent_sleeping_rigid | ent_living, &pContact, 0, (geom_colltype_player << rwi_colltype_bit) | rwi_stop_at_pierceable);
...
//------------------------------------------------------------------------
void CGameRules::ForceScoreboard(const bool force)
{
	SAFE_HUD_FUNC(ForceScoreBoard(force));
}

//------------------------------------------------------------------------
void CGameRules::FreezeInput(const bool freeze)
{
...
//------------------------------------------------------------------------
bool CGameRules::IsProjectile(const EntityId id) const
{
	return g_pGame->GetWeaponSystem()->GetProjectile(id) != nullptr;
}

//------------------------------------------------------------------------
void CGameRules::AbortEntityRespawn(const EntityId entityId, const bool destroyData)
{
	const TEntityRespawnMap::iterator it = m_respawns.find(entityId);
	if (it != m_respawns.end())
		m_respawns.erase(it);

	if (destroyData)
	{
		const TEntityRespawnDataMap::iterator dit = m_respawndata.find(entityId);
		if (dit != m_respawndata.end())
			m_respawndata.erase(dit);
	}
}
...
//------------------------------------------------------------------------
void CGameRules::ScheduleEntityRemoval(EntityId entityId, const float timer, const bool visibility)
{
	if (!gEnv->bServer || m_pGameFramework->IsEditing())
		return;
...
//------------------------------------------------------------------------
void CGameRules::AbortEntityRemoval(const EntityId entityId)
{
	const TEntityRemovalMap::iterator it = m_removals.find(entityId);
...
void CGameRules::SendRadioMessage(const EntityId sourceId, const int msg)
{
	...
	const RadioMessageParams params(sourceId, msg);
...
{
	const float timeRemaining = GetRemainingGameTime();
	const int   mins          = static_cast<int>(timeRemaining / 60.0f);
	const int   secs          = static_cast<int>(timeRemaining - mins * 60);
	CryLogAlways("time remaining: %d:%02d", mins, secs);
}

void CGameRules::OnAction(const ActionId& actionId, const int activationMode, const float value)
{
	if (m_pRadio)
		m_pRadio->OnAction(actionId, activationMode, value);
}

void CGameRules::ReconfigureVoiceGroups(const EntityId id, const int old_team, int new_team)
{
...
void CGameRules::ForceSynchedStorageSynch(const int channel)
{
	if (!gEnv->bServer)
		return;
...
void CGameRules::PlayerPosForRespawn(const CPlayer* pPlayer, const bool save)
{
...
void CGameRules::SPNotifyPlayerKill(EntityId targetId, EntityId weaponId, bool bHeadShot) const
{
...
bool CGameRules::NetSerialize(const TSerialize ser, const EEntityAspects aspect, uint8 profile, int flags)
{
	switch (aspect)
	{
...
void CGameRules::CreateRestrictedItemList(const char* restrictedItems)
{
	...
	{
		const int slen = strlen(itemString);
		for (int i = 0; i < slen; ++i)
		{
			int    k = i;
			string thisItemName;
			while (k < slen && itemString[k] != ',' && itemString[k] != ';' && itemString[k] != ' ')
			{
				++k;
			}
...
void DbgPlotter::Reset()
{
	...
	const float ratio = (m_maxY - m_minY) / (m_maxX - m_minX);
...
void DbgPlotter::WriteImg(const EntityId ownerId)
{
	if (!m_isEnabled)
		return;
...
void DbgPlotter::PlotSpawnPoints()
{
	...
	for (; count >= 0; --count)
	{
		g_dbgPlotter.Plot(pGameRules->GetSpawnLocation(count), eT_SpawnPoint);
	}
}

void DbgPlotter::PlotTeam(const EntityId entID, const bool enemy)
{
	if (!m_isEnabled)
		return;
	const CGameRules*   pGameRules = static_cast<CGameRules*>(gEnv->pGame->GetIGameFramework()->GetIGameRulesSystem()->GetCurrentGameRules());
...
void DbgPlotter::PlotBox(const float x, const float y, const float halfSz, const EDbgPlotTypes entType)
{
	if (!m_isEnabled)
		return;
	const float x0(max(m_minX, (x - halfSz)));
	const float x1(min(m_maxX, (x + halfSz)));
	const float y0(max(m_minY, (y - halfSz)));
	const float y1(min(m_maxY, (y + halfSz)));
...
void DbgPlotter::PlotCircle(const float x, const float y, const float r, const EDbgPlotTypes entType)
{
	if (!m_isEnabled)
		return;
	...
	{
		const float x2 = min(max(0.0f, x + r * sinf(angle)), m_maxX);
		const float y2 = min(max(0.0f, y + r * cosf(angle)), m_maxY);

		Plot(x2, y2, entType, false);
	}
}

void DbgPlotter::Plot(const EntityId entID, const EDbgPlotTypes entType)
{
	if (!m_isEnabled)
		return;
...
void DbgPlotter::Plot(const float x, const float y, const EDbgPlotTypes entType, const bool bigPixel)
{
	if (!m_isEnabled)
		return;
...
//------------------------------------------------------------------------
bool CGameRules::IsSpawnUsedTouch(const EntityId spawnId)
{
	const float curTime = GetISystem()->GetITimer()->GetCurrTime();

	const TSpawnPointUseTime::iterator it = m_SpawnPointUseTime.find(spawnId);
	if (it != m_SpawnPointUseTime.end())
	{
		const float diff = curTime - it->second;
		it->second = curTime;
		if (diff > 2.f)
			return false;
		return true;
...
//------------------------------------------------------------------------
bool CGameRules::IsSpawnUsed(const EntityId spawnId) const
{
	const float curTime = GetISystem()->GetITimer()->GetCurrTime();

	const TSpawnPointUseTime::const_iterator it = m_SpawnPointUseTime.find(spawnId);
	if (it != m_SpawnPointUseTime.end())
	{
		const float diff = curTime - it->second;
		if (diff > 2.f)
