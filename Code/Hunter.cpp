...
bool CHunter::CreateCodeEvent(SmartScriptTable& rTable)
{
	...
	if (event && !strcmp(event, "dropObject"))
	{
		if (m_pGrabHandler)
...
void CHunter::Revive(const bool fromInit)
{
	...
	if (pCharacter)
	{
		...
		if (m_IKLimbIndex[1] < 0)
			m_IKLimbIndex[1] = GetIKLimbIndex("frontRightTentacle");
		if (m_IKLimbIndex[2] < 0)
			m_IKLimbIndex[2] = GetIKLimbIndex("backLeftTentacle");
		if (m_IKLimbIndex[3] < 0)
	...
	for (int i = 0; i < 4; ++i)
	{
		...
		if (TentacleGoingDown(i) && TentacleInTheAir(i))
			if (!Airborne())
			{
				...
				// transform local limb position to the world
				m_footGroundPos[i] = GetEntity()->GetSlotWorldTM(pLimb->characterSlot) * lPos;
		...
		if (m_footTouchesGround[i] == true)
			SmoothCD(m_footTouchesGroundSmooth[i], m_footTouchesGroundSmoothRate[i], frameTime, static_cast<f32>(footOnGround), 0.002f);
		else
		...
		if (m_footGroundPos[i].GetLength() > 0.0f)
			m_footGroundPosLast[i] = m_footGroundPos[i];

		// t0 стремится к 1.0 когда тентакля стоит на земле
		// t0 стремится к 0.0 когда тентакля не находится на земле
		f32 t0 = m_footTouchesGroundSmooth[i];

		// t1 стремится к 0.0 когда тентакля стоит на земле
		// t1 стремится к 1.0 когда тентакля не находится на земле
		...
		{
			auto r = static_cast<uint8>(t0 * 255.0f);
			auto g = static_cast<uint8>(t1 * 255.0f);

			auto redAndGreen = ColorB(r, g, 0, 255);
			auto blue = ColorB(0, 0, 255, 255);

			GetISystem()->GetIRenderer()->GetIRenderAuxGeom()->DrawSphere(vFinalLimbPosition, 0.52f, redAndGreen);
			GetISystem()->GetIRenderer()->GetIRenderAuxGeom()->DrawSphere(m_footGroundPosLast[i], 0.5f, blue);
		}
...
int CHunter::GetBoneID(int ID, int slot) const
{
	if (m_boneIDs[ID] < 0)
	{
		...
		{
		case BONE_HEAD:
			strcpy(boneStr, "face_bigass_gun");
			break;
		case BONE_WEAPON:
			strcpy(boneStr, "face_bigass_gun");
			break;
