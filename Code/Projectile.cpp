...
-------------------------------------------------------------------------
History:
- 18:10:2005   14:14 : Created by Mбrcio Martins
		...
		case ePT_Particle:
			{
				if (m_pAmmoParams->pParticleParams)
				{
					...
					if (!m_initial_dir.IsZero() && !gEnv->bServer)
						m_pAmmoParams->pParticleParams->heading=m_initial_dir.GetNormalizedSafe();
				}
...
void CProjectile::OnHit(const HitInfo& hit)
{
	...
	if(hit.targetId==GetEntityId() && m_hitPoints>0 && !m_destroying)
	{
		...
		if(m_hitPoints<=0)
		{
			// This is a fix for the issue where one player detonating this projectile by shooting it,
			// while the owner was nearby would cause the owner to suicide, instead of crediting the kill to whoever
			// caused the explosion
			if (m_pAmmoParams->pExplosion && hit.shooterId && gEnv->bServer)
				m_ownerId=hit.shooterId;
