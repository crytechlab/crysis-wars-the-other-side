...
-------------------------------------------------------------------------
History:
- 12:04:2006   17:22 : Created by Mбrcio Martins
...
COffHand::COffHand() : m_currentState(eOHS_INIT_STATE),
					   m_mainHand(NULL),
	...
	else
	{
		...
		if (pAttachment)
		{
			...
			else
			{
				...
				// other slots should be invisible
				if (slot != eIGS_Aux0)
					GetEntity()->SetSlotFlags(eIGS_Aux0, GetEntity()->GetSlotFlags(eIGS_Aux0) & ~ENTITY_SLOT_RENDER);
				if (slot != eIGS_Aux1)
					GetEntity()->SetSlotFlags(eIGS_Aux1, GetEntity()->GetSlotFlags(eIGS_Aux1) & ~ENTITY_SLOT_RENDER);
				if (slot != eIGS_OwnerLooped)
					GetEntity()->SetSlotFlags(eIGS_OwnerLooped, GetEntity()->GetSlotFlags(eIGS_OwnerLooped) & ~ENTITY_SLOT_RENDER);
				if (slot != eIGS_Owner)
					GetEntity()->SetSlotFlags(eIGS_Owner, GetEntity()->GetSlotFlags(eIGS_Owner) & ~ENTITY_SLOT_RENDER);
			}
		}
	}
}
