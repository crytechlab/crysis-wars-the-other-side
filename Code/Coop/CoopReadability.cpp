...
void CCoopReadability::SendSoundToActor(ISound* pSound, EntityId actorId)
{
	...
	{
		//TheOtherSide
		//Replace CActor to CTOSActor
		if (CTOSActor* pActor = static_cast<CTOSActor*>(gEnv->pGame->GetIGameFramework()->GetIActorSystem()->GetActor(actorId)))
		{
			...
			CTOSActor::PlayReadabilitySoundParams params;
			...
			pActor->GetGameObject()->InvokeRMI(CTOSActor::ClPlayReadabilitySound(), params, eRMI_ToAllClients | eRMI_NoLocalCalls);
		}
		//~TheOtherSide
	}
}
...
bool CCoopReadability::SendSoundToClosestActor(ISound* pSound)
{
	...
	if (sem == eSoundSemantic_AI_Readability)
	{
		...
		if (pClosestActor)
		{
		   ...
		   // params.vOffset = pSound->GetPosition() - pClosestActor->GetEntity()->GetPos();
		   // params.nSemantic = pSound->GetSemantic();
			pClosestActor->GetGameObject()->InvokeRMI(CTOSActor::ClPlayReadabilitySound(), params, eRMI_ToAllClients | eRMI_NoLocalCalls);
			return true;
