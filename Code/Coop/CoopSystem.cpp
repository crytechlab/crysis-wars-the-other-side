...
bool CCoopSystem::Initialize()
{
	...
	hCryCooperativeModule = CryLoadLibrary("..\\Mods\\CrysisTheOtherSide\\Bin64\\CryCooperative.dll");
	if (hCryCooperativeModule)
	{
		const auto CreateCooperativeSystem = reinterpret_cast<PTR_CreateCooperativeSystem>(GetProcAddress(hCryCooperativeModule, "CreateCooperativeSystem"));
	...
	{
		throw std::runtime_error("CryCooperative.dll wasn't loaded");
	}
}
