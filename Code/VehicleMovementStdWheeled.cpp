...
const float LOAD_RAMP_TIME = 0.2f;
const float LOAD_RELAX_TIME = 0.25f;
...
void CVehicleMovementStdWheeled::ProcessMovement(const float deltaTime)
{
    ...
    if (GetMovementType() != IVehicleMovement::eVMT_Amphibious && 
        m_PhysDyn.submergedFraction >= m_submergedRatioMax && 
        m_damage >= 0.99f)
    {
        m_action.pedal = 0.0f;
    }
