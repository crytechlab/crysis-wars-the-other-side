...
CDebrisSpawner::~CDebrisSpawner() { Reset(); }
...
void CDebrisSpawner::Update(const float deltaTime) const {}
...
CAlien::~CAlien()
{
	...
	if (pCharacter)
	{
		...
		pCharacter->GetISkeletonPose()->SetPostPhysicsCallback(nullptr, nullptr);
	}
...
void CAlien::PostPhysicalize()
{
	...
	const float jlim = g_pGameCVars->g_tentacle_joint_limit;
	if (jlim >= 0)
...
void CAlien::UpdateView(SViewParams& viewParams)
{
	...
	{
		const float thirdPersonDistance(g_pGameCVars->cl_tpvDist);
		const float thirdPersonYaw(g_pGameCVars->cl_tpvYaw);
...
void CAlien::ProcessRotation(float frameTime)
{
	...
	//	if (m_stats.inAir && IsZeroG())
	{
		...
		if (m_input.viewVector.len2() > 0.0f)
		{
			...
			const Quat eyeTargetQuat(eyeTarget);
			const Quat currQuat(m_eyeMtx);
			m_eyeMtx = Matrix33(Quat::CreateSlerp(currQuat.GetNormalized(), eyeTargetQuat, min(frameTime * 12.0f, 1.0f)));
		}

		if (m_input.viewVector.len2() > 0.0f)
		{
			const Vec3 lookat = m_eyeMtx.GetColumn(1);
			const Vec3 orient = m_viewMtx.GetColumn(1);
...
void CAlien::ProcessBonesRotation(ICharacterInstance* pCharacter, const float frameTime)
{
	...
	if (pCharacter)
	{
		...
		{
			const int32 idx = pCharacter->GetISkeletonPose()->GetJointIDByName("root");
			if (idx > -1)
			{
				const Vec3 rootPos(pCharacter->GetISkeletonPose()->GetAbsJointByID(idx).t * m_stats.physicsAnimationRatio);
...
void CAlien::Draw(const bool draw)
{
...
//------------------------------------------------------------------------
void CAlien::Reset(const bool toGame)
{
	if (m_pGroundEffect)
...
void CAlien::RagDollize(const bool fallAndPlay)
{
...
void CAlien::SetSearchBeamQuat(const Quat& rot)
{
	...
	const Quat rotCharInv = Quat(GetEntity()->GetSlotLocalTM(0, false)).GetInverted();
	const Quat rotBoneInv = pChar->GetISkeletonPose()->GetAbsJointByID(m_searchbeam.pAttachment->GetBoneID()).q.GetInverted();
...
void CAlien::SetActorMovementCommon(SMovementRequestParams& control)
{
	...
	if (control.bExactPositioning || bExactPos)
		...
		{
			// todo: determine a better threshold
			const float dist((GetEntity()->GetWorldPos() - control.remainingPath[0].vPos).GetLengthSquared());
			if (dist < .3f * .3f)
...
void CAlien::SetActorStance(SMovementRequestParams& control, int& actions)
{
	...
	const float timeEstimate = 0.25f; // if we are the point in timeEstime seconds, start to go vert already.
...
void CAlien::SetAnimTentacleParams(pe_params_rope& pRope, const float animBlend)
{
	...
	else
	{
		...
		else
		{
			...
			const float value(1.0f - animBlend);
...
void CAlien::StanceChanged(const EStance last)
{
	const float delta(GetStanceInfo(last)->modelOffset.z - GetStanceInfo(m_stance)->modelOffset.z);
	if (delta > 0.0f)
...
void CAlien::UpdateDebugGraphs()
{
	...
	m_pDebugHistoryManager->LayoutHelper("EntID", nullptr, true, -1000000, 1000000, 1000000, -1000000, 0.0f, 1.0f);

	const bool showReqVelo = true;
	m_pDebugHistoryManager->LayoutHelper("ReqVelo", nullptr, showReqVelo, -20, 20, 0, 5, 0.0f, 0.0f);
	m_pDebugHistoryManager->LayoutHelper("ReqVeloX", nullptr, showReqVelo, -20, 20, -5, 5, 1.0f, 0.0f);
	m_pDebugHistoryManager->LayoutHelper("ReqVeloY", nullptr, showReqVelo, -20, 20, -5, 5, 2.0f, 0.0f);
	m_pDebugHistoryManager->LayoutHelper("ReqVeloZ", nullptr, showReqVelo, -20, 20, -5, 5, 3.0f, 0.0f);
