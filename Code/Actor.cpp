/*************************************************************************
 ...
 -------------------------------------------------------------------------
  History:
  - 7:10:2004   14:48 : Created by Mбrcio Martins
...
//------------------------------------------------------------------------
CActor::CActor()
	: m_pAnimatedCharacter(0), m_isClient(false), m_health(100.0f), m_maxHealth(0), m_pMovementController(0), m_stance(STANCE_NULL), m_desiredStance(STANCE_NULL), m_previousStance(STANCE_NULL), m_stanceChangeTime(0.0f), m_zoomSpeedMultiplier(1.0f), m_frozenAmount(0.f), m_screenEffects(0), m_bHasHUD(false), m_autoZoomInID(0), m_autoZoomOutID(0), m_saturationID(0), m_hitReactionID(0), m_inCombat(false), m_enterCombat(false), m_pGrabHandler(NULL), m_teamId(0), m_lastItemId(0), m_pInventory(0), m_sleepTimer(0.0f), m_sleepTimerOrg(0.0f), m_currentFootID(BONE_FOOT_L), m_lostHelmet(0), m_pWeaponAM(0), m_suicideDelay(-1.f), m_dropWpnPendingId(0), m_dropWpnWaitTime(0.f), m_reviveNoReactionTime(0.f)
{
...
void CActor::UpdateScriptStats(SmartScriptTable &rTable)
{
	...
	if (pStats)
	{
		if (!gEnv->bMultiplayer)
		{
			// these don't seem to be needed in MP.
...
int CActor::GetBoneID(int ID, int slot) const
{
	if (m_boneIDs[ID] < 0)
	{
		...
		case BONE_BIP01:
			...
			break;
		case BONE_SPINE:
			pBoneStr = "Bip01 Spine";
			break;
		case BONE_SPINE2:
			pBoneStr = "Bip01 Spine2";
			break;
		case BONE_SPINE3:
			pBoneStr = "Bip01 Spine3";
			break;
		case BONE_HEAD:
			pBoneStr = "Bip01 Head";
			break;
		case BONE_EYE_R:
			pBoneStr = "eye_right_bone";
			break;
		case BONE_EYE_L:
			pBoneStr = "eye_left_bone";
			break;
		case BONE_WEAPON:
			pBoneStr = "weapon_bone";
			break;
		case BONE_FOOT_R:
			pBoneStr = "Bip01 R Foot";
			break;
		case BONE_FOOT_L:
			pBoneStr = "Bip01 L Foot";
			break;
		case BONE_ARM_R:
			pBoneStr = "Bip01 R Forearm";
			break;
		case BONE_ARM_L:
			pBoneStr = "Bip01 L Forearm";
			break;
		case BONE_CALF_R:
			pBoneStr = "Bip01 R Calf";
			break;
		case BONE_CALF_L:
			pBoneStr = "Bip01 L Calf";
			break;
		}
...
//[kirill] need this to fix IK blending when leaving MG on vehicles
...
IMPLEMENT_RMI(CActor, SvRequestPlayNetworkedAnimation)
{
	// Описываем здесь всё, что будет выполняться на сервере

	GetGameObject()->InvokeRMI(ClPlayNetworkedAnimation(), params, eRMI_ToRemoteClients);
...
bool CActor::SetAnimationInput(const char *inputID, const char *value)
{
	...
	if (strcmp(value, m_sLastNetworkedAnim) != 0)
	{
		this->GetGameObject()->InvokeRMI(ClPlayNetworkedAnimation(),
										 SPlayNetworkedAnimationParams(bSignal ? EAnimationMode::AIANIM_SIGNAL : EAnimationMode::AIANIM_ACTION, value), eRMI_ToRemoteClients);
...
{
	SPlayNetworkedAnimationParams params;
	params.sAnimation = action;
	params.nMode = looping ? AIANIM_ACTION : AIANIM_SIGNAL;

	if (gEnv->bClient)
	{
		GetGameObject()->InvokeRMI(SvRequestPlayNetworkedAnimation(), params, eRMI_ToServer);
	}
	else
	{
		GetGameObject()->InvokeRMI(ClPlayNetworkedAnimation(), params, eRMI_ToRemoteClients);
