...
bool CHUD::Init()
{
	...
	TOS_RECORD_EVENT(0, STOSGameEvent(eEGE_HUDInit, "", true));
...
void CHUD::SpawnPointInvalid()
{
	if (m_animSpawnCycle.IsLoaded())
	{
		...
		DisplayGameStartMessage("@spawnpointlost");
	}
}
//-----------------------------------------------------------------------------------------------------
...
void CHUD::OnPostUpdate(float frameTime)
{
	...
	if (m_bShow && (m_cineState == eHCS_None || gEnv->bMultiplayer) && pPlayer && !m_bInMenu)
	{
		...
		if (gEnv->bMultiplayer && m_currentGameRules == EHUD_POWERSTRUGGLE)
		{
			if (g_pGame->GetGameRules() && g_pGame->GetGameRules()->IsPlayerActivelyPlaying(pPlayer->GetEntityId()))
			{
				if (!m_animSpawnCycle.IsLoaded() && !GetModalHUD())
				{
					if (pPlayer->GetHealth() <= 0)
					{
						if (m_spawnWarningTimer <= 0.0f)
						{
							DisplayGameStartMessage("@ui_team_spawn_start", NULL, NULL, NULL);
							m_spawnWarningTimer = SPAWN_WARNING_TIMER;
						}
						else
							m_spawnWarningTimer -= frameTime;
					}
				}
			}
		}
...
void CHUD::GameOver(int localWinner, int winnerTeam, EntityId id)
{
	...
	EntityId checkId = 0;
	if (pGameRules->IsPlayerActivelyPlaying(pActor->GetEntityId()))
		checkId = pActor->GetEntityId();
	...
	if (pGameRules->GetTeamCount() > 1)
	{
		...
		{
			int checkTeamId = pGameRules->GetTeam(checkId);
			if (localWinner == 0)
			{
				//do nothing, message already set to draw
			}
