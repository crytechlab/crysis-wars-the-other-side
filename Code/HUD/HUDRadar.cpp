...
CHUDRadar::CHUDRadar()
{
	...
	m_soundIdCounter = static_cast<unsigned>(1 << 26);
...
void CHUDRadar::ShowSoundOnRadar(Vec3 pos, float intensity)
{
	...
	if (m_soundIdCounter > static_cast<unsigned>(1 << 29))
		m_soundIdCounter = static_cast<unsigned>(1 << 26);
}
...
struct ArrayFillHelper
{
	ArrayFillHelper(IFlashPlayer* pFlashPlayer, const char* varName)
...
template <typename T, EFlashVariableArrayType flashType, int NUM_VALS>
int FillUpDoubleArray(ArrayFillHelper<T, flashType, NUM_VALS>* fillHelper, double a, double b, double c, double d, double e, double f, double g = 100.0f, double h = 100.0f, double i = 0.0f, double j = 0.0f, double k = 0.0f)
{
...
void CHUDRadar::Update(float fDeltaTime)
{
	...
	if (isMultiplayer && g_pGameCVars->hud_showRadarObjectiveIcons)
	{
		...
		for (int t = 0; t < m_buildingsOnRadar.size(); ++t)
		{
			...
			if (id)
			{
				...
				if (pEntity)
				{
					...
					numOfValues += ::FillUpDoubleArray(&entityValues, pEntity->GetId(), type, (fX - lowerBoundX) / dimX, (fY - lowerBoundY) / dimY, 0.0f, FriendOrFoe(gEnv->bMultiplayer, playerTeam, pEntity, g_pGame->GetGameRules()), sizeScale * 100.0f, 100.0f);
				}
			}
		}
	}
	...
	for (int t = 0; t < m_tempEntitiesOnRadar.size(); ++t)
	{
		...
		if (pEntity && !pEntity->IsHidden())
		{
			...
			numOfValues += ::FillUpDoubleArray(&entityValues, temp.m_id, temp.m_type, (fX - lowerBoundX) / dimX, (fY - lowerBoundY) / dimY, 180.0f + RAD2DEG(fAngle), FriendOrFoe(gEnv->bMultiplayer, playerTeam, pEntity, g_pGame->GetGameRules()), sizeScale * 25.0f, fAlpha * 100.0f);
		}
	}
	...
	if (pActor)
	{
		...
		float radarRatio = (static_cast<float>(m_miniMapEndX[m_mapId] - m_miniMapStartX[m_mapId]) / fRadius) * 50.0f;
		...
		if (pEntity)
		{
			...
			if (inAAA) //check whether it's an unknown, flying airplane when you are in an AAA (show all airplanes > 3m over ground)
			{
				...
				if (eType == EHeli)
				{
					...
					if (pos.z - gEnv->p3DEngine->GetTerrainZ(static_cast<int>(pos.x), static_cast<int>(pos.y)) > 5.0f)
					{
...
void CHUDRadar::UpdateCompassStealth(CActor* pActor, float fDeltaTime)
{
	...
	if (!gEnv->bMultiplayer)
	{
		...
		gEnv->pAISystem->GetDetectionLevels(nullptr, levels);
	...
	if (gEnv->bMultiplayer) //shows the player coordinates in MP
	{
		...
		int   index = min(static_cast<int>(fY / 0.125f), 7);
		sprintf(strCoords, m_coordinateToString[index].c_str(), static_cast<int>(value));
...
bool CHUDRadar::ScanObject(EntityId id)
{
	...
	if (CheckObject(pEntity, id != m_lookAtObjectID, id != m_lookAtObjectID) && !IsOnRadar(id, 0, m_entitiesOnRadar.size() - 1))
	{
	...
	if (m_scannerTimer > 0.0f)
	{
		...
		if (m_scannerTimer <= 0.0f)
		{
			...
			if (m_scannerObjectID)
				...
				if (m_pActorSystem->GetActor(m_scannerObjectID) || m_pVehicleSystem->GetVehicle(m_scannerObjectID))
...
bool CHUDRadar::CheckObject(IEntity* pEntity, bool checkVelocity, bool checkVisibility)
{
	...
	if ((uiFlags & ENTITY_FLAG_ON_RADAR) && pPE && (!checkVisibility || (checkVisibility && fSquaredDistance < 1000000.0f && bPointVisible)))
	{
		...
		if (checkVisibility)
		{
			...
			IPhysicalEntity* pSkipEnt = pActor ? pActor->GetEntity()->GetPhysics() : nullptr;
...
Vec2i CHUDRadar::GetMapGridPosition(float x, float y)
{
	...
	retVal.y = static_cast<int>(cry_ceilf(outX * 8.0f));
	retVal.x = static_cast<int>(cry_ceilf(outY * 8.0f));
...
{
	float posX = std::max(0.0f, inX - std::min(static_cast<float>(m_miniMapStartX[m_mapId]), static_cast<float>(m_miniMapEndX[m_mapId])));
	float posY = std::max(0.0f, inY - std::min(static_cast<float>(m_miniMapStartY[m_mapId]), static_cast<float>(m_miniMapEndY[m_mapId])));
	float temp = posX;
	posX = posY;
	posY = temp;
	outX = std::max(static_cast<float>(m_startOnScreenX), static_cast<float>(m_startOnScreenX) + (std::min(posX * m_mapDivX, 1.0f) * static_cast<float>(m_mapDimX)));
	outY = std::max(static_cast<float>(m_startOnScreenY), static_cast<float>(m_startOnScreenY) + (std::min(posY * m_mapDivY, 1.0f) * static_cast<float>(m_mapDimY)));
	if (flashCoordinates)
	{
		outX = (outX - static_cast<float>(m_startOnScreenX)) / static_cast<float>(m_mapDimX);
		outY = (outY - static_cast<float>(m_startOnScreenY)) / static_cast<float>(m_mapDimY);
	}
	return true;
}
...
void CHUDRadar::ComputeMiniMapResolution()
{
	...
	float width = static_cast<float>(gEnv->pRenderer->GetWidth());
	float height = static_cast<float>(gEnv->pRenderer->GetHeight());
	...
	m_startOnScreenX = static_cast<int>(800 - ((800 - m_startOnScreenX) / 600.0f * 800 / width * height));
	m_endOnScreenX = static_cast<int>(800 - ((800 - m_endOnScreenX) / 600.0f * 800 / width * height));
...
void CHUDRadar::RenderMapOverlay()
{
		...
		for (int e = 0; e < m_tempEntitiesOnRadar.size(); ++e)
		{
			...
			if (pTempActor = m_pActorSystem->GetActor(id))
			{
				if (IVehicle* pVehicle = pTempActor->GetLinkedVehicle())
				{
					...
					{
						GetPosOnMap(pVehicle->GetEntity(), fX, fY);
						numOfValues += FillUpDoubleArray(&entityValues, pVehicle->GetEntity()->GetId(), ChooseType(pVehicle->GetEntity(), false), fX, fY, 270.0f - RAD2DEG(pVehicle->GetEntity()->GetWorldAngles().z), FriendOrFoe(isMultiplayer, team, pVehicle->GetEntity(), pGameRules), 100, 100, iOnScreenObjective == pVehicle->GetEntity()->GetId(), iCurrentSpawnPoint == pVehicle->GetEntity()->GetId());
						drawnVehicles[pVehicle->GetEntityId()] = true;
					}
				}
				else
				{
					GetPosOnMap(pTempActor->GetEntity(), fX, fY);
					numOfValues += FillUpDoubleArray(&entityValues, pActor->GetEntity()->GetId(), ChooseType(pTempActor->GetEntity(), false), fX, fY, 270.0f - RAD2DEG(pTempActor->GetEntity()->GetWorldAngles().z), FriendOrFoe(isMultiplayer, team, pTempActor->GetEntity(), pGameRules), 100, 100, iOnScreenObjective == pTempActor->GetEntity()->GetId(), iCurrentSpawnPoint == pTempActor->GetEntity()->GetId());
				}
			}
			else if (IVehicle* pVehicle = m_pVehicleSystem->GetVehicle(id))
			{
	...
	//draw story entities (icons with text)
	{
		for (int e = 0; e < m_storyEntitiesOnRadar.size(); ++e)
		{
			...
			{
				GetPosOnMap(pEntity, fX, fY);
				numOfValues += FillUpDoubleArray(&entityValues, id, m_storyEntitiesOnRadar[e].m_type, fX, fY, 270.0f - RAD2DEG(pEntity->GetWorldAngles().z), ENeutral, 100, 100, iOnScreenObjective == id, iCurrentSpawnPoint == id);
...
FlashRadarFaction CHUDRadar::FriendOrFoe(bool multiplayer, int playerTeam, IEntity* entity, CGameRules* pGameRules)
{
	...
	if (multiplayer)
	{
		...
		val = static_cast<FlashRadarFaction>(friendly);
	}
...
void CHUDRadar::StartBroadScan(bool useParameters, bool keepEntities, Vec3 pos, float radius)
{
	...
	if (m_bsUseParameter) //this is a custom scan
	{
...
void CHUDRadar::ScanProximity(Vec3& pos, float& radius)
{
	...
	query.box = AABB(Vec3(pos.x - radius, pos.y - (radius * 1.33f), pos.z - radius), Vec3(pos.x + radius, pos.y + (radius * 1.33f), pos.z + radius));
...
bool CHUDRadar::RadarBounds_Inside(const Vec2& pos, Vec2& intersectionPoint)
{
	...
	static const Vec2 radarPolygon[] = {Vec2(center + Vec2(-58, 17.0f)), Vec2(center + Vec2(-58, -42)), Vec2(center + Vec2(62, -42)), Vec2(center + Vec2(62, 32.15f)), Vec2(center + Vec2(46.25f, 45)), Vec2(center + Vec2(-46.75f, 45)), Vec2(center + Vec2(-46.75f, 25.8f))};
