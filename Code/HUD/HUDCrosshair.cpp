...
void CHUDCrosshair::Update(const float fDeltaTime)
{
	if (m_bBroken)
		return;
	...
	if (pWeapon)
	{
		...
		if (const IFireMode* pFireMode = pWeapon->GetFireMode(pWeapon->GetCurrentFireMode()))
		{
		...
		if (pSlaveWeapon)
			if (const IFireMode* pSlaveFireMode = pSlaveWeapon->GetFireMode(pSlaveWeapon->GetCurrentFireMode()))
			{
...
void CHUDCrosshair::SetUsability(const int usable, const char* actionLabel, const char* paramA, const char* paramB)
{
	m_iUsable = usable;
	...
	if (actionLabel)
	{
		if (paramA)
		{
			CryFixedStringT<64> paramLocA;
			CryFixedStringT<64> paramLocB;
			if (paramA[0] != '@')
			{
				paramLocA = "@";
...
int CHUDCrosshair::GetUsability() const
{
	return m_iUsable;
}
...
bool CHUDCrosshair::IsFriendlyEntity(const IEntity* pEntity)
{
	...
	const IActor* pActor = g_pGame->GetIGameFramework()->GetIActorSystem()->GetActor(pEntity->GetId());
...
void CHUDCrosshair::UpdateCrosshair()
{
	...
	else
	{
		...
		{
			const EntityId uiCenterId = pClientActor->GetGameObject()->GetWorldQuery()->GetLookAtEntityId();
			if (uiCenterId)
	...
	{
		const CWeapon* pWeapon = g_pHUD->GetCurrentWeapon();
		if (pWeapon)
		{
			...
			if (pClass == pClaymoreClass || pClass == pAVMineClass)
				if (const IFireMode* pfm = pWeapon->GetFireMode(pWeapon->GetCurrentFireMode()))
					if (!pfm->IsFiring())
	...
	if (m_animInterActiveIcons.GetVisible())
	{
		...
		if (pItem)
		{
			...
			if (pWeapon)
			{
				const CItem::SStats stats = pItem->GetStats();
		...
		{
			const EntityId offHandId    = pClientActor->GetInventory()->GetItemByClass(CItem::sOffHandClass);
...
void CHUDCrosshair::SetOpacity(const float opacity)
{
...
void CHUDCrosshair::SelectCrosshair(IItem* pItem)
{
	...
	if (pClientActor)
		if (g_pGameCVars->hud_crosshair != 0)
		{
			...
			{
				if (pItem->GetEntity()->GetClass() == CItem::sRocketLauncherClass || pItem->GetEntity()->GetClass() == CItem::sTACGunFleetClass)
				{
					SetCrosshair(0);
					bSpecialCrosshairSet = true;
				}
				...
				else if (g_pHUD->GetSelectedFiremode() == 31 || g_pHUD->GetSelectedFiremode() == 26 || g_pHUD->GetSelectedFiremode() == 29 || g_pHUD->GetSelectedFiremode() == 28 || g_pHUD->GetSelectedFiremode() == 27 || g_pHUD->GetSelectedFiremode() == 30) //auto grenade launcher: needs fire mode because of pickup
				{
					SetCrosshair(15);
					bSpecialCrosshairSet = true;
				}
...
void CHUDCrosshair::Break(const bool state)
{
	m_bBroken = state;
