...
const const char* const ITEM_ARMS_ATTACHMENT_NAME = "arms_fp";

const const char* const ITEM_DESELECT_POSE = "nw";

const const char* const ITEM_FIRST_PERSON_TOKEN = "fp";
const const char* const ITEM_THIRD_PERSON_TOKEN = "tp";
...
public:
	...
	{
		SEntityProperties() : hitpoints(0), pickable(true), mounted(false), physics(false), usable(false)
		{
...
public:
	...
	{
		return (!m_bonusAccessoryAmmo.empty() || m_params.attachment_gives_ammo);
	}

	virtual bool IsAmmoPickup() const
	{
		return false;
	}
	...
	void SetActionSuffix(const char* suffix)
	{
	...
	{
		return m_params.auto_droppable;
	}
