...
enum ENanoDisableFlag
{
	NANODISABLE_GENERAL = 0,
	NANODISABLE_EMP,
	NANODISABLE_FROZEN,
	NANODISABLE_NUMENTRIES
};
...
struct SNanoCloak
{
	...
	ILINE uint8 GetState() const
	{
		return m_mode * m_active;
	}

	ILINE float GetVisualDamp() const
	{
		return m_active ? m_visualDamp : 1.0f;
	}

	ILINE float GetSoundDamp() const
	{
		return m_active ? m_soundDamp : 1.0f;
	}

	ILINE float GetHeatDamp() const
	{
		return m_active ? m_heatDamp : 1.0f;
	}

	ILINE bool IsActive() const
	{
		return m_active;
	}

	ILINE void SetType(ENanoCloakMode mode)
	{
		m_mode = mode;
	}

	ILINE ENanoCloakMode GetType() const
	{
		return m_mode;
	}
...
private:
	void Update(CNanoSuit* pNano) const;
...
public:
	...
	static bool           AssignNanoMaterialToEntity(const IEntity* pEntity, const SNanoMaterial* pNanoMaterial);

	CNanoSuit();
	~CNanoSuit();

	void Activate(bool activate, ENanoDisableFlag flag = NANODISABLE_GENERAL, float activationTime = 0.0f);
	void ActivateMode(ENanoMode mode, bool active);
	void SetModeDefect(ENanoMode mode, bool defect);
	bool IsActive() const;

	bool IsModeActive(const ENanoMode mode) const
	{
		return (m_featureMask & (1 << mode)) ? true : false;
	}

	void Init(CPlayer* owner)
	{
		Reset(owner);
	}
	...
	ENanoAction PendingAction() const
	{
	...
	void SetParams(const SmartScriptTable& rTable, bool resetFirst);
	...
	ILINE float GetHealthRegenRate() const
	{
		return m_healthRegenRate;
	}

	ILINE const CPlayer* GetOwner() const
	{
		return m_pOwner;
	}

	ILINE const SNanoCloak* GetCloak() const
	{
		return &m_cloak;
	}

	ILINE bool IsNightVisionEnabled() const
	{
		return (m_bNightVisionEnabled && m_active);
	};
	ILINE void EnableNightVision(const bool enable)
	{
		m_bNightVisionEnabled = enable;
	};
	//get sprinting multiplier for ground movement
	float GetSprintMultiplier(bool strafing) const;
	void  SetCloak(bool on, bool force = false);

	float GetHealthRegenDelay() const
	{
		return m_healthRegenDelay;
	}
...
private:
	...
	static int GetButtonFromMode(ENanoMode mode);
