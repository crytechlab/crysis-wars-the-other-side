...
function SinglePlayer:IsUsable(srcId, objId)
	...
	if (obj.IsUsable) then
		...
		--TheOtherSide
		local cantUse = src:IsDead() or (src.actor:GetSpectatorMode() ~= 0) or src.actorStats.isFrozen
		local zeus = src.actor:IsZeus()

		if (src and src.actor and (cantUse or zeus)) then
			return 0;
		end
		--~TheOtherSide
...
----------------------------------------------------------------------------------------------------
function SinglePlayer:CreateExplosion(shooterId, weaponId, damage, pos, dir, radius, angle, pressure, holesize, effect,
									  effectScale, minRadius, minPhysRadius, physRadius)
	if (not dir) then
		dir = g_Vectors.up;
	end
	...
	self.game:ServerExplosion(shooterId or NULL_ENTITY, weaponId or NULL_ENTITY, damage, pos, dir, radius, angle,
		pressure, holesize, effect, effectScale, nil, minRadius, minPhysRadius, physRadius);
end
...
function SinglePlayer:ViewShake(player, pos, radius, amount, duration, frequency, source)
	...
	if (deltaDist > 0.0) then
		...
		player.actor:SetViewShake({ x = 2 * g_Deg2Rad * amt, y = 2 * g_Deg2Rad * amt, z = 2 * g_Deg2Rad * amt },
			{ x = 0.02 * amt, y = 0.02 * amt, z = 0.02 * amt }, halfDur + halfDur * (deltaDist / radius), 1 / 20, 4);
	...
	if (collider) then
		if (collider.GetForeignCollisionMult) then
			...
			if (debugColl > 0 and foreignMult ~= 1) then
				Log("<%s>: collider <%s> has ForeignCollisionMult %.2f", entity:GetName(), collider:GetName(),
					foreignMult);
			end
		end
	end
...
function SinglePlayer:ProcessActorDamage(hit)
	...
	if (self.game:DebugCollisionDamage() > 0) then
		Log("<%s> hit damage: %d // absorbed: %d // health: %d", target:GetName(), hit.damage,
			hit.damage * self:GetDamageAbsorption(target, hit), health);
	end
...
function SinglePlayer:ProcessDeath(hit)
	...
	if (hit.target.id == g_localActorId) then
		...
		if (hit.shooterId == g_localActorId) then
			if (not AI.Hostile(hit.target.id, g_localActorId, false)) then -- should never happen atm, because we prevent death when player shot friend
...
function SinglePlayer:LogHit(hit, extended, dead)
	if (dead) then
		Log("'%s' hit '%s' for %d with '%s'... *DEADLY*", EntityName(hit.shooter), EntityName(hit.target),
			hit.damage or 0, (hit.weapon and hit.weapon:GetName()) or "");
	else
		Log("'%s' hit '%s' for %d with '%s'...", EntityName(hit.shooter), EntityName(hit.target), hit.damage or 0,
			(hit.weapon and hit.weapon:GetName()) or "");
	end
...
function SinglePlayer.Server:OnHit(hit)
	...
	if (self:IsMultiplayer() or ((not hit.target.actor) or (not hit.target.actor:IsPlayer()))) then
		...
		if (hit.type == "melee" and headshot == false) then
			material_type = "torso";
		end

		hit.damage = math.floor(0.5 +
		self:CalcDamage(material_type, hit.damage, self:GetDamageTable(hit.shooter, hit.target), hit.assistance));
	end
...
function SinglePlayer:GetCollisionEnergy(entity, hit)
	...
	if (bCollider) then -- non-static
		...
		if (debugColl > 0) then
			...
			CryAction.PersistantArrow(hit.pos, 1.5, ScaleVector(hit.normal, sgn(v0dotN)), g_Vectors.v010, "CollDamage",
				5.0);
			CryAction.PersistantArrow(hit.pos, 1.3, ScaleVector(hit.normal, sgn(v1dotN)), g_Vectors.v100, "CollDamage",
				5.0);

			if (v0Sq > 2 * 2 or v1Sq > 2 * 2) then
				Log("normal velocities: rel %.1f, <%s> %.1f / <%s> %.1f", math.sqrt(vrelSq), entity:GetName(), v0dotN,
					hit.target and hit.target:GetName() or "none", v1dotN);
