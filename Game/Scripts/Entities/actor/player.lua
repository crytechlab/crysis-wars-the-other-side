...
--		self.actor:SetExtensionActivation("Interactor",1);
		--self.actor:SetExtensionActivation("HUD", 1);
--	self.actor:SetExtensionParams("Interactor",{useHoverTime=0.05,unUseHoverTime=0.2,messageHoverTime=0.05,longHoverTime=5,queryMethods="rb"})
--	self.actor:SetExtensionParams("WorldQuery",{proximityRadius=5})

	-- TOMAS: here is the code to enable to test the network sound stuff
	-- (SNH: not needed any more)
	--self.actor:SetExtensionActivation("VoiceListener",1)
...
--	HUD:Reset();
	...
	if AI then AI.ResetParameters(self.id, self.Properties, self.PropertiesInstance, nil,self.melee) end;
