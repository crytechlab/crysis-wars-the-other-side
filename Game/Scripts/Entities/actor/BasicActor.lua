...
{	
	...
	gameParams =
	{
		...
		strafeMultiplier = 1.0,--speed is multiplied by this ammount when strafing in SP
		strafeMultiplierMP = 0.75,--speed is multiplied by this ammount when strafing in MP
...
function BasicActor:Reset()
	...
	self.lastPSVehicleId = nil;
	self.AI.theVehicle = nil;
	
	self.lastExitedVehicleId = nil;
	self.lastExitedVehicleTime = nil;
...
function BasicActor:DoPainSounds(dead)
	...
	if (self.actor:IsPlayer()) then
		...
		if (not dead) then		
			local freq = System.GetCVar("g_painSoundGap");
			if (self.lastPainSound and ((_time-self.lastPainTime<freq) or Sound.IsPlaying(self.lastPainSound))) then
				return;
			end
		end
...
function BasicActor:ScriptEvent(event,value,str)
	...
	elseif (event == "landed") then
		...
		if(value > System.GetCVar("pl_fallDamage_SpeedSafe")) then
