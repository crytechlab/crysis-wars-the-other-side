		...
		if (hit.shooter and hit.shooter == g_localActor) then
			if (tonumber(System.GetCVar("g_useHitSoundFeedback")) > 0) then
				if (hit.target and hit.target.class == "Alien") then
					self:PlaySoundEvent("sounds/physics:bullet_impact:alien_flesh_feedback", g_Vectors.v000,
						g_Vectors.v010, SOUND_2D, SOUND_SEMANTIC_PLAYER_FOLEY);
				else
					self:PlaySoundEvent("sounds/physics:bullet_impact:alien_shell_feedback", g_Vectors.v000,
						g_Vectors.v010, SOUND_2D, SOUND_SEMANTIC_PLAYER_FOLEY);
				end
			end
		end
...
function BasicAlien:MeleeCustom(entity, pos, dir)
	...
	if (entity.actor) then
		...
		entity.actor:AddAngularImpulse({ x = randomF(-math.pi * 0.01, math.pi * 0.01), y = 0, z = -math.pi * 0.1 }, 0.0,
			1.5);
	end
end
		...
		if (self.Properties.Vulnerability.dmgRatio < 1) then
			...
			if (newRatio > props.dmgRatio and prevRatio <= props.dmgRatio) then
				...
				if (matEff and matEff.effect) then
					local attName = self.actor:AttachVulnerabilityEffect(0, hit.partId, hit.pos, matEff.radius,
						matEff.effect, matEff.attachments);
				end
