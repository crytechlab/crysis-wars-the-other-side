...
----------------------------------------------------------------------------------------------------
--  History:
--  - 16:8:2005   10:38 : Created by Mбrcio Martins
--
----------------------------------------------------------------------------------------------------
DestroyableObject =
{
...
function DestroyableObject:Explode()
	...
	if (NumberToBool(self.Properties.bExplode)) then
		...
		local explo_pos = pos;
		g_gameRules:CreateExplosion(self.shooterId, self.id, expl.Damage, explo_pos, expl.Direction, expl.Radius, nil,
			expl.Pressure, expl.HoleSize, expl.Effect, expl.EffectScale, expl.MinRadius, expl.MinPhysRadius,
			expl.PhysRadius);
	end

	-- play the dead sound after explosion
	if (self.dead ~= true) then
		self:PlaySoundEvent(self.Properties.Sounds.sound_Dying, g_Vectors.v000, g_Vectors.v001, 0,
			SOUND_SEMANTIC_MECHANIC_ENTITY);
	end
	self:PlaySoundEvent(self.Properties.Sounds.sound_Dead, g_Vectors.v000, g_Vectors.v001, 0,
		SOUND_SEMANTIC_MECHANIC_ENTITY);
...
function DestroyableObject.Server:OnHit(hit)
	...
	elseif (hit.type == "collision") then
		pass = NumberToBool(vul.bCollision); damage = damage * mult.fCollision;
	elseif (hit.type == "bullet") then
		pass = NumberToBool(vul.bBullet); damage = damage * mult.fBullet;
...
DestroyableObject.Client.Alive =
{
	OnBeginState = function(self)
		self:PlaySoundEvent(self.Properties.Sounds.sound_Alive, g_Vectors.v000, g_Vectors.v001, 0,
			SOUND_SEMANTIC_MECHANIC_ENTITY);
	end,
}
