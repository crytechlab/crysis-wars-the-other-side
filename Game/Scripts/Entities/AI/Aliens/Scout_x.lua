...
function Scout_x:Kill(ragdoll, shooterId, weaponId)
	...
	local pathName = AI.GetNearestPathOfTypeInRange(self.id, self:GetPos(), 10000.0,
		AIAnchorTable.ALIEN_COMBAT_DEATH_PATH, 0.0, 0);
	...
	if (self.iAutoDestructTimer == 1) then
		...
		if (self.waterCrash ~= nil) then
			...
			if (self.waterCrash > 3000) then
				...
				self:PlaySoundEvent(self.gameParams.destruct_charge_sound, g_Vectors.v000, g_Vectors.v010, 0,
...
function Scout_x.Client:OnCollision(hit)
	...
	if (vel > 8 * 8) then
		...
		if (self.firstCrash) then
			...
			self:PlaySoundEvent(self.gameParams.destruct_gound_sound, g_Vectors.v000, g_Vectors.v010, 0,
				SOUND_SEMANTIC_EXPLOSION);
		end
		...
		if (not self.firstCrash) then
			...
			-- MR: remove after review (move to MatFX)
			self:PlaySoundEvent(self.gameParams.death_explode_sound, g_Vectors.v000, g_Vectors.v010, 0,
				SOUND_SEMANTIC_EXPLOSION);
		end
	end
end
