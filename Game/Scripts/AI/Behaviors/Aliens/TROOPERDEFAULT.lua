...
--	Log(">>>>>>>>>>>>>>>>>> JUMP_ON_ROCK!!!! <<<<<<<<<<<<<<<<<<<<<<<<");
		...
		if (entity.actor:HasShieldProjector()) then
...
--		if(Trooper_IsJumping(entity)) then 
	...
	CHECK_FIRE = function (self,entity,sender)
		if(AI.GetGroupTargetCount(entity.id,true)>0) then 
			entity:InsertSubpipe(0,"start_fire");
		else
			entity:InsertSubpipe(0,"stop_fire");
		end
...
--			AI.PushGoal("trace",1,1);
	...
	--------------------------------------------------
	SET_SWIMMING_MODE_NORMAL = function(self,entity,sender)
		if (entity.actor) then
			entity.actor:SetSwimmingMode(SWIMMING_MODE_NORMAL);
			entity:EnableMaterialLayer(false, MTL_LAYER_WET);
		end
	end,
	
	--------------------------------------------------
	SET_SWIMMING_MODE_DIVE = function(self,entity,sender)
		if (entity.actor) then
			entity.actor:SetSwimmingMode(SWIMMING_MODE_DIVE);
			entity:EnableMaterialLayer(false, MTL_LAYER_WET);
		end
	end,
	
	--------------------------------------------------
	SET_SWIMMING_MODE_EMERGE = function(self,entity,sender)
		if (entity.actor) then
			entity.actor:SetSwimmingMode(SWIMMING_MODE_EMERGE);
			entity:EnableMaterialLayer(true, MTL_LAYER_WET);
		end
	end,
	
	--------------------------------------------------
	FINISH_EMERGE = function(self,entity,sender)
		local pos=entity:GetPos();
		pos.z=pos.z+1.6;
		Particle.SpawnEffect("expansion_alien_fx.alien_trails.trooper_emerge_from_water",pos,g_Vectors.v001,entity.id);
		Script.SetTimerForFunction( 5000 , "AIBehaviour.TROOPERDEFAULT.FinishEmerge", entity );
	end,
	
	--------------------------------------------------
	FinishEmerge = function(entity)
		if (entity) then
			entity.actor:SetSwimmingMode(SWIMMING_MODE_NORMAL);
			entity:EnableMaterialLayer(false, MTL_LAYER_WET);
		end
	...
	-- MKII
	--------------------------------------------------
	EndThreatened = function(entity)
		-- Log(">>> Trooper MKII Ending Threatened");
		entity.threatTimer=nil;
		if( entity.inCloseRange == false ) then

			---------------------------------------------
			-- use COVER2's behaviours
			---------------------------------------------
	...
	--------------------------------------------------
	TR2_DODGE_DONE = function (self, entity)
		entity:SelectPipe(0,"do_nothing");
	...
	---------------------------------------------
	OnOutOfAmmo = function (self,entity, sender)
		AI.Signal(SIGNALFILTER_SENDER,1,"MKII_RELOAD",entity.id);
	end,
	---------------------------------------------
	MKII_RELOAD = function(self, entity, sender, data)
		if (entity.AI.tryingToReload and entity.AI.tryingToReload == true) then
			return;
		end
		
		entity.AI.tryingToReload = true;
		-- This was the mechanism inside reload behavior, but not reliable!
		-- entity:SelectPipe(AIGOALPIPE_HIGHPRIORITY,"tr2_take_cover_reload");
		AI.Signal(SIGNALFILTER_SENDER,1,"MKII_HANDLE_RELOAD",entity.id);
	end,
	------------------------------------------------------------------------
	MKII_HANDLE_RELOAD = function(self,entity,sender)
		-- entity:SelectPipe(0,"do_nothing");
	...
	OnReloadDone = function(self,entity)
		...
		if( target == AITARGET_ENEMY and targetDist and targetDist < 5 ) then
			Trooper_GoClose(entity);
		else
			AI.Signal(SIGNALFILTER_SENDER,0,"CONTROL_COVER",entity.id,1);
		end			
	end,
	------------------------------------------------------------------------

	-- SCANNER_EFFECT = function(self, entity, sender, data)
	-- 	if (data) then
	-- 		if (data.iValue == 0) then
	-- 			entity:ShutDownBeams();
	-- 		elseif (data.iValue == 1) then
	-- 			entity:EnableDiscoveryBeam(true, -1, -1, -1);
	-- 		elseif (data.iValue == 2) then
	-- 			entity:EnableSearchBeam(true, -1, -1, -1);
	-- 		elseif (data.iValue == 3) then
	-- 			entity:EnableSeekBeam(true, -1, -1, -1);
	-- 		elseif (data.iValue == 4) then
	-- 			entity:EnableTargetSearchBeam(true, -1, -1, -1);
	-- 		elseif (data.iValue == 5) then
	-- 			entity:EnableTargetBeam(true, -1, -1, -1);
	-- 		elseif (data.iValue == 6) then
	-- 			entity:Reload(true);
	-- 		end
	-- 	end
	-- end,
	...
	------------------------------------------------------------------------
	-- debug
	CHECK_TROOPERMKII_GROUP = function(self,entity,sender)
		if (entity and entity.Properties.bAllowLeader and entity.Properties.bAllowLeader == 1) then
			AI.WarningAggressive("TROOPER MK II ".. sender:GetName().. " IS IN SAME GROUP WITH BASIC TROOPER "..entity:GetName()..", groupid = "..AI.GetGroupOf(entity.id));
		end
	end,
