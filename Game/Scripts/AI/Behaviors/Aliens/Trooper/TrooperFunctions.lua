...
function Trooper_Jump(trooper, targetPos, startAnim, useEvent, addAngle, bUseSpecialAnim, maxAngle)
	...
	if (t and t > 0.7) then
		if (startAnim) then
			trooper.actor:SetParams({ jumpTo = targetPos, jumpVelocity = velocity, jumpTime = t, jumpStart = startAnim, jumpLand = true, useAnimEvent =
			useEvent, useSpecialAnim = bUseSpecialAnim });
		else
			trooper.actor:SetParams({ jumpTo = targetPos, jumpVelocity = velocity, jumpTime = t, jumpLand = true, useAnimEvent =
			useEvent, useSpecialAnim = bUseSpecialAnim });
		end
	...
	if (target) then
		...
		if (diffz > -1.5 and diffz < 1.5) then
			...
			if (targetSpeed < 1) then
				...
				--TheOtherSide
				entity:RequestCloakTurnOff();
				--~TheOtherSide			  	entity:MeleeAttack(target);
	...
	if (curTime - AIBlackBoard.lastJumpMeleeTime > 5) then
		...
		if (target) then
			...
			if (t and t > 0.1) then
				...
				local t1 = AI.CanJumpToPoint(entity.id, jumpPos, 1, 20, AI_JUMP_ON_GROUND + AI_JUMP_CHECK_COLLISION,
					entityAI.jumpVel, nil, targetPos);
...
function Trooper_PerformSecondMeleeJump(entity)
	...
	if (t) then
		...
		local entityAI = entity.AI;
		entity.actor:SetParams({ jumpTo = entityAI.jumpPos, jumpVelocity = entityAI.jumpVel, jumpTime = entityAI
		.jumpTime });
	end
...
Trooper_Search = function(entity, pos)
	...
	AI.Signal(SIGNALFILTER_LEADER, 1, "ORDER_SEARCH", entity.id, g_SignalData);
end
