...
--TheOtherSide
	...
	TOS_AREA_GET_NEAREST_HIDESPOT = function(self,entity,sender,data)
		local areaSpotName = AI.FindObjectOfType(entity:GetPos(), 100, AIAnchorTable.TOS_SA_CENTRE_SPOT, AIFAF_INCLUDE_DEVALUED);
		if (areaSpotName) then
			...
			if (areaSpot) then
				...
				local anchorName = AI.FindObjectOfType(areaSpot:GetPos(), areaRadius, AIAnchorTable.COMBAT_HIDESPOT, AIFO_CHOOSE_RANDOM+AIFO_NONOCCUPIED+AIFO_NONOCCUPIED_REFPOINT);
	...
	TOS_GET_NEAREST_HIDESPOT = function(self,entity,sender,data)
		...
		local anchorName = AI.FindObjectOfType(position, radius, AIAnchorTable.COMBAT_HIDESPOT, AIFO_FACING_TARGET+AIFO_CHOOSE_RANDOM+AIFO_NONOCCUPIED
