...
AIBehaviour.DEFAULT = {
	...
	GET_RANDOM_POINT = function(self, entity, sender, data)
		...
		if (entity.actor) then
			pos = entity.actor:GetProbablySearchLocation();
		elseif (entity.vehicle) then
			local driver = System.GetEntity(entity:GetDriverId());
			if (driver and driver.actor) then
				pos = driver.actor:GetProbablySearchLocation();
			end
		end

		AI.SetRefPointPosition(entity.id, pos);
		--System.LogAlways("GET_RANDOM_POINT ref("..AI.GetRefPointPosition(entity.id).x..","..AI.GetRefPointPosition(entity.id).y..")");
	end,
	DETACHED_FOLLOWPATH = function(self, entity, sender, data)
		local pathfind = data.point.x;
		local reverse = data.point.y;
		local startNearest = data.point.z;
		local loops = data.fValue;

		g_StringTemp1 = "DETACHED_FOLLOWPATH";
		-- if(pathfind > 0) then
		...
		entity:SelectPipe(0, g_StringTemp1, nil, data.iValue);
	end,
	DETACHED_GOTO_POINT = function(self, entity, sender, data)
		local pathfind = data.point.x;
		local reverse = data.point.y;
		local startNearest = data.point.z;
		local loops = data.fValue;
	...
	TOS_AREA_GET_NEAREST_HIDESPOT = function(self, entity, sender, data)
		local areaSpotName = AI.FindObjectOfType(entity:GetPos(), 100, AIAnchorTable.TOS_SA_CENTRE_SPOT,
			AIFAF_INCLUDE_DEVALUED);
		if (areaSpotName) then
			local areaSpot = System.GetEntityByName(areaSpotName);
			if (areaSpot) then
				local areaRadius = areaSpot.Properties.radius;
				local anchorName = AI.FindObjectOfType(areaSpot:GetPos(), areaRadius, AIAnchorTable.COMBAT_HIDESPOT,
					AIFO_CHOOSE_RANDOM + AIFO_NONOCCUPIED + AIFO_NONOCCUPIED_REFPOINT);

				if (anchorName) then
					local hideSpot = System.GetEntityByName(anchorName);
					if (hideSpot) then
						AI.SetRefPointPosition(entity.id, hideSpot:GetPos());
					end
				end
			end
		end
	end,
	TOS_GET_NEAREST_HIDESPOT = function(self, entity, sender, data)
		local radius = data.iValue;
		local position = data.point;
		local anchorName = AI.FindObjectOfType(position, radius, AIAnchorTable.COMBAT_HIDESPOT,
			AIFO_FACING_TARGET + AIFO_CHOOSE_RANDOM + AIFO_NONOCCUPIED
			+ AIFO_NONOCCUPIED_REFPOINT + AIFO_USE_BEACON_AS_FALLBACK_TGT);
	...
	TOS_UNIT_INCOVER = function(self, entity, sender, data)
		entity:SelectPipe(0, "tos_commander_ord_goto_sub_hide_incover");
	end,
	TOS_SHOOT_AT = function(self, entity, sender, data)
		AI.CreateGoalPipe("tos_shoot_at");
		AI.PushGoal("tos_shoot_at", "locate", 1, "refpoint");
		AI.PushGoal("tos_shoot_at", "+firecmd", 1, FIREMODE_FORCED, AILASTOPRES_USE);
		AI.PushGoal("tos_shoot_at", "+timeout", 1, data.fValue);
		AI.PushGoal("tos_shoot_at", "firecmd", 1, 0);
		AI.PushGoal("tos_shoot_at", "signal", 1, -1, "TOS_ON_END_SHOOTING", 0);

		entity:SelectPipe(AIGOALPIPE_SAMEPRIORITY, "do_it_standing");
		entity:SelectPipe(AIGOALPIPE_SAMEPRIORITY, "tos_shoot_at");

		--AI.Signal(SIGNALFILTER_SENDER, 1, "RETURN_TO_FIRST",entity.id);
	end,
	TOS_VEHICLE_SHOOT_AT = function(self, entity, sender, data)
		AI.CreateGoalPipe("tos_shoot_at");
		AI.PushGoal("tos_shoot_at", "locate", 1, "refpoint");
		AI.PushGoal("tos_shoot_at", "+firecmd", 1, FIREMODE_FORCED, AILASTOPRES_USE);
		AI.PushGoal("tos_shoot_at", "+timeout", 1, data.fValue);
	...
	TOS_QUICKLY_FOLLOW_END = function(self, entity, sender, data)
		AI.CreateGoalPipe("tos_quickly_follow_end");
		AI.PushGoal("tos_quickly_follow_end", "timeout", 1, 0.1, 0.2);

		entity:SelectPipe(0, "tos_quickly_follow_end");
	end,
	TOS_COOLDOWN_AND_GOTO = function(self, entity, sender, data)
		entity:SelectPipe(0, "ord_cooldown");
		entity:SelectPipe(0, "ord_goto");
	end,
	TOS_COOLDOWN = function(self, entity, sender, data)
		entity:TriggerEvent(AIEVENT_CLEAR);
		--entity:InsertSubpipe(0,"devalue_target");
		-- entity:CancelSubpipe();
		entity:SelectPipe(0, "do_nothing");
		entity:InsertSubpipe(0, "ord_cooldown");
		entity:InsertSubpipe(0, "clear_all");
		entity:InsertSubpipe(0, "do_it_standing");
	end,
	TOS_GOTO_GUARD = function(self, entity, sender, data)
		entity:SelectPipe(0, "ord_ready_guard");
	end,
	TOS_ON_END_SHOOTING = function(self, entity, sender, data)
		entity:SelectPipe(0, "tos_on_end_shooting");
	end,
	TOS_DO_NOTHING = function(self, entity, sender, data)
		entity:SelectPipe(0, "do_nothing");
	end,
	TOS_ONRESET = function(self, entity, sender, data)
		entity:OnResetAI();
	end,
	--~TheOtherSide
	...
	MOUNTED_WEAPON_USABLE = function(self, entity, sender, data)
		...
		if (weapon and AI.IsMountedWeaponUsableWithTarget(entity.id, weapon.id, 7, entity.AI.SkipTargetCheck)) then
			...
			--TheOtherSide
			if (parent and parent.vehicle and not g_gameRules.game:IsConquestGamemode()) then
	...
	RUSH_AI = function(self, entity, sender, data)
		AI.SetRefPointPosition(entity.id, data.point);

		entity:SelectPipe(0, "rush_ai");
		entity:Readibility("AI_AGGRESSIVE", 1);
	end,
	...
	-- 		--System.LogAlways("RETURN_TO_FIRST");
	-- --		entity.EventToCall = "OnSpawn";	
	...
	START_CONVERSATION = function(self, entity, sender)
		...
		local conv = AI_ConvManager:GetRandomConversation(entity.ConversationName, entity.AI.ConvType,
			entity.AI.ConvInPlace, entity.AI.ConvPartecipants);
	...
	MAKE_STUNNED_ANIMATION = function(self, entity, sender)
		if (entity.BLINDED_ANIM_COUNT) then
			...
			Hud:AddMessage("==================UNACCEPTABLE ERROR====================");
			Hud:AddMessage("Entity " ..
			entity:GetName() .. " tried to make blinded anim, but no blindXX anims for his character.");
			Hud:AddMessage("==================UNACCEPTABLE ERROR====================");
		end
	end,
	...
	ACT_GOTO = function(self, entity, sender, data)
		if (data and data.point) then
			...
			--AI.PushGoal(g_StringTemp1, "pathfind", 1, "refpoint");
			--AI.PushGoal(g_StringTemp1, "branch", 0, "NO_PATH", IF_LASTOP_FAILED );
			AI.PushGoal(g_StringTemp1, "+stick", 1, data.point2.x, AILASTOPRES_USE + AI_REQUEST_PARTIAL_PATH, STICK_BREAK,
				data.fValue);                                                                                                   -- noncontinuous stick
			AI.PushGoal(g_StringTemp1, "branch", 0, "END", BRANCH_ALWAYS);
			--AI.PushLabel(g_StringTemp1, "NO_PATH" );
			--AI.PushGoal(g_StringTemp1, "signal", 1, 1, "CANCEL_CURRENT",0);
			AI.PushLabel(g_StringTemp1, "END");
	...
	ACT_ENTERVEHICLE = function(self, entity, sender, data)
		...
		if (data.fValue < 1 or data.fValue > numSeats) then
			entity.AI.mySeat = entity.AI.theVehicle:RequestGunnerSeat(entity.id);
		else
	...
	ACT_ANIMEX = function(self, entity, sender, data)
		if (data) then
			...
			AI.PushGoal("action_animEx", "+animtarget", 0, data.iValue2, data.ObjectName, data.point.x, data.fValue,
				data.point2.x);
