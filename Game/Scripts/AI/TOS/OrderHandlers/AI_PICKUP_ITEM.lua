...
function AI_PICKUP_ITEM(entity, item, speed)
    ...
    if (not orderRefEnt) then
        ...
        spawnParams.position = item:GetWorldPos()
       ...
       --  LogAlways("<AI_GOTO> order tag point spawned: %s, ai: %s", orderRefEnt:GetName(), tostring(AI.HasAI(orderRefEnt.id)))
        ...
        orderRefEnt:SetWorldPos(item:GetWorldPos())
        -- LogAlways("<AI_GOTO> order tag point updated: %s", orderRefName)
    end

    entity.orderRefEnt = orderRefEnt
    entity.pickupItem = item
    entity.currentExecutedOrder = EOrders.AI_PICKUP_ITEM

    TOS_AI.BeginGoalPipe("ai_pickup_item")
        TOS_AI.PushGoal(GO.SIGNAL, 1, AISIGNAL_DEFAULT, "AI_PICKUP_ITEM_STARTED", SIGNALFILTER_SENDER)
        ...
        TOS_AI.PushGoal(GO.SIGNAL, 1, AISIGNAL_DEFAULT, "AI_PICKUP_ITEM_ENDED", SIGNALFILTER_SENDER)
    TOS_AI.EndGoalPipe()

    TOS_AI.SendSignal(SIGNALFILTER_SENDER, AISIGNAL_DEFAULT, "GO_TO_TOS_OBEY", entity.id)
    TOS_AI.SendSignal(SIGNALFILTER_SENDER, AISIGNAL_PROCESS_NEXT_UPDATE, "START_AI_PICKUP_ITEM", entity.id)
end
...
function CLEAR_DATA_AI_PICKUP_ITEM(entity)
    ...
    -- LogAlways("entity.pickupItem == "..entity.pickupItem:GetName())

    entity.actor:PickUpItem(entity.pickupItem.id)
    entity.pickupItem = nil
...
function AIBehaviour.DEFAULT:START_AI_PICKUP_ITEM(entity, sender)
    ...
    TOS_AI.SelectPipe(AIGOALPIPE_LOOP, entity, "ai_pickup_item", entity.orderRefEnt.id)
end
...
function AIBehaviour.DEFAULT:AI_PICKUP_ITEM_ENDED(entity, sender)
    ...
    CLEAR_DATA_AI_PICKUP_ITEM(entity)
